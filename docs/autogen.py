...
def generate():
    sources_dir = keras_dir / 'docs/sources'
    ...
    readme = read_file(keras_dir / 'README.md')
    index = read_file(template_dir / 'index.md')
    index = index.replace('{{autogenerated}}', readme[readme.find('##'):])
    with open(sources_dir / 'index.md', 'w') as f:
        f.write(index)
    ...
    for page_data in PAGES:
        ...
        path = sources_dir / page_name
        if path.exists():
            template = read_file(path)
            assert '{{autogenerated}}' in template, ('Template found for '
                                                     + str(path) +
    ...
    shutil.copyfile(keras_dir / 'CONTRIBUTING.md', sources_dir / 'contributing.md')
    copy_examples()
