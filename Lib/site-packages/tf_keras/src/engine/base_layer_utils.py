...
# If True, TF-Keras will not evaluate the constant-foldable inputs to tf op
...
def _create_keras_history_helper(tensors, processed_ops, created_layers):
    ...
    """
        raise ValueError(
            ...
            "outputs are currently not supported by TF-Keras automatic "
...
def is_in_keras_graph():
    """Returns if currently executing inside of a TF-Keras graph."""
...
def is_in_tf_function():
    ...
    if not tf.inside_function():
        return False
    # Check if inside TF-Keras FuncGraph.
...
def check_graph_consistency(tensor=None, method="add_loss", force_raise=False):
    """Checks that tensors passed to `add_*` method match the TF-Keras graph.
...
def enable_v2_dtype_behavior():
    """Enable the V2 dtype behavior for TF-Keras layers.
...
def disable_v2_dtype_behavior():
    """Disables the V2 dtype behavior for TF-Keras layers.
