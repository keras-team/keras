...
from tf_keras.src.engine import functional_utils
...
from tf_keras.src.saving.legacy.saved_model import network_serialization
from tf_keras.src.saving.legacy.saved_model import utils as saved_model_utils
...
def reconstruct_from_config(config, custom_objects=None, created_layers=None):
    ...
    def _deserialize_keras_tensors(kwargs, layer_map):
        """Deserializes TF-Keras Tensors passed to `call`.."""

        def _deserialize_keras_tensor(t):
            """Deserializes a single TF-Keras Tensor passed to `call`."""
    ...
    def process_node(layer, node_data):
        ...
        for input_data in tf.nest.flatten(node_data):
            ...
            if input_data[0] != node_module._CONSTANT_VALUE:
                ...
                # We received a constant w/ no TF-Keras history attached,
...
def get_network_config(network, serialize_layer_fn=None, config=None):
    ...
    # To be removed after full affected g3 user migration to TF-Keras V3 Saving.
    ...
    # Preserve external TF-Keras compat for Models with single input.
    ...
    # Preserve external TF-Keras compat for Models with single output.
