...
from tf_keras.src.saving.legacy import model_config
...
from tf_keras.src.saving.legacy.saved_model.serialized_attributes import (
    CommonEndpoints,
)
...
def load(path, compile=True, options=None):
    """Loads TF-Keras objects from a SavedModel.

    Any TF-Keras layer or model saved to the SavedModel will be loaded back
    ...
    if not metadata.nodes:
        # When there are no TF-Keras objects, return the results from the core
        # loader
...
class KerasObjectLoader:
    """Loader that recreates TF-Keras objects (e.g.
    ...
    def finalize_objects(self):
        """Finish setting up TF-Keras objects.
...
def _finalize_saved_model_layers(layers):
    """Runs the final steps of loading TF-Keras Layers from SavedModel."""
...
def _unable_to_call_layer_due_to_serialization_issue(
    ...
    TF-Keras Model/Layer serialization is relatively relaxed because SavedModels
...
def _finalize_config_layers(layers):
    """Runs the final steps of loading TF-Keras Layers from config."""
    for layer in layers:
...
def recursively_deserialize_keras_object(config, module_objects=None):
    """Deserialize TF-Keras object from a nested structure."""
    if isinstance(config, dict):
        ...
        raise ValueError(
            "Unable to decode TF-Keras layer config. Config should be a "
