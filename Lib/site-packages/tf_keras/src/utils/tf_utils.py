...
def set_random_seed(seed):
    ...
    You can use this utility to make almost any TF-Keras program fully
...
def shape_type_conversion(fn):
    ...
    def wrapper(instance, input_shape):
        ...
        # This preserves compatibility with external TF-Keras.
...
def is_symbolic_tensor(tensor):
    ...
    if isinstance(tensor, tf.Tensor):
        ...
        # Variables that are output of a TF-Keras Layer in Functional API mode
...
def register_symbolic_tensor_type(cls):
    ...
    allows non-`Tensor` objects to be plumbed through TF-Keras layers.
...
def convert_variables_to_tensors(values):
    ...
    This is a TF-Keras version of `convert_variables_to_tensors` in TensorFlow
    variable_utils.py.
...
def assert_no_legacy_layers(layers):
    """Prevent tf.layers.Layers from being used with TF-Keras.
