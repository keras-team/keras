...
When training on systems with multiple devices (CPUs, GPUs, TPUs), you may want to
control which device tensors are allocated on. Keras provides the `keras.device()`
context manager for backend-agnostic device placement.

### Using the device context manager

The `keras.device()` context manager works across all backends (TensorFlow, JAX, PyTorch):
"""
...
- **Memory management**: Keeping large tensors on CPU to save GPU memory
- **Data preprocessing**: Processing data on CPU before moving to GPU for training
- **Multi-GPU setups**: Explicitly controlling which GPU receives which tensors

### Device naming conventions

The device name format is `"device_type:device_index"`:

- `"cpu:0"` - First CPU
- `"gpu:0"` - First GPU (works across backends)
- `"gpu:1"` - Second GPU

For PyTorch backend, `"gpu"` is automatically converted to `"cuda"`.

### Example: CPU preprocessing with GPU training
...
# Train on GPU
with keras.device("gpu:0"):
    model.fit(x_processed, y_train, epochs=10)
```
