...
torch_pairs = [
    ...
    (
        "torch  LLM  generate 32tok  eager",
        "keras[torch]  LLM  generate 32tok  eager",
        "LLM gen eager  ",
    ),
    (
        "torch  LLM  generate 32tok  compile",
        "keras[torch]  LLM  generate 32tok  compile",
        "LLM gen compile",
    ),
]
...
print("  " + "-" * 66)
print(
    "  {:<18s}  {:>10s}  {:>14s}  {:>10s}".format(
        "Op", "PyTorch", "Keras[torch]", "Overhead"
    )
)
print(
    "  {:<18s}  {:>10s}  {:>14s}  {:>10s}".format(
...
for pt_k, kt_k, label in torch_pairs:
    if pt_k in combined and kt_k in combined:
        ...
        print(
            "  {}  {:8.2f} ms  {:12.2f} ms  {:8.2f}x".format(
                label, pt_v, kt_v, kt_v / pt_v
            )
        )
...
jax_pairs = [
    ...
    (
        "jax  LLM  forward  eager",
        "keras[jax]  LLM  forward  eager",
        "LLM fwd eager  ",
    ),
    (
        "jax  LLM  forward  jit",
        "keras[jax]  LLM  forward  jit",
        "LLM fwd jit    ",
    ),
    (
        "jax  LLM  generate 32tok  eager",
        "keras[jax]  LLM  generate 32tok  eager",
        "LLM gen eager  ",
    ),
    (
        "jax  LLM  generate 32tok  jit+scan",
        "keras[jax]  LLM  generate 32tok  jit",
        "LLM gen jit    ",
    ),
]
...
print(
    "  {:<18s}  {:>10s}  {:>14s}  {:>10s}".format(
        "Op", "Pure JAX", "Keras[jax]", "Overhead"
    )
)
print(
    "  {:<18s}  {:>10s}  {:>14s}  {:>10s}".format(
