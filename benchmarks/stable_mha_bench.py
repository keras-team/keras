...
for name, dense in [("query", mha_new._query_dense), ("key", mha_new._key_dense),
                     ("value", mha_new._value_dense), ("output", mha_new._output_dense)]:
...
import statistics
N = 5000
times_old = [timeit.timeit(lambda: mha_old(x, x), number=1) * 1e3 for _ in range(N)]
times_new = [timeit.timeit(lambda: mha_new(x, x), number=1) * 1e3 for _ in range(N)]
...
print(f"Mean  : einsum={statistics.mean(times_old):.3f}ms, matmul={statistics.mean(times_new):.3f}ms")
print(f"p5    : einsum={times_old[N//20]:.3f}ms, matmul={times_new[N//20]:.3f}ms")
print(f"p95   : einsum={times_old[int(N*0.95)]:.3f}ms, matmul={times_new[int(N*0.95)]:.3f}ms")
