...
print(
    f"Forward-only loop:      {fwd_only_time:.2f} ms  ({GEN} static forwards without concat)"
)
...
print(
    f"Per-step ops only:      {ops_overhead:.2f} ms  (argmax+cast+concat × {GEN})"
)
...
print(
    f"Keras ops overhead:     {ops_overhead - torch_ops:.2f} ms total, {(ops_overhead - torch_ops) / GEN:.3f} ms/step"
)
