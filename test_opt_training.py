...
from keras.distribution import ModelParallel
...
def train_opt_model_parallel():
    ...
    print(
        "  Testing end-to-end with permanent backend fixes (ALL LAYERS SHARDED)"
    )
    ...
    print(
        f"Creating and building model within distribution scope on RANK {rank}..."
    )
    with distribution.scope():
        ...
        dummy_input = {
            "token_ids": np.random.randint(0, 1000, (1, 32)).astype("int32"),
        ...
        for v in model.variables:
            if hasattr(v.value, "placements"):
                has_shard = any(
                    isinstance(p, dt.Shard) for p in v.value.placements
                )
                if has_shard:
                    shard_count += 1
                    if rank == 0 and (
                        "/kernel" in v.path
                        or "layer_norm" in v.path
                        or "token_embedding" in v.path
                    ):
