...
# with spaces are gradually fed in. 
# 
...
def paint_text(text, w, h):
    ...
    with cairo.Context(surface) as context:
        ...
        context.select_font_face(random.choice(fonts), cairo.FONT_SLANT_NORMAL,
                                 random.choice([cairo.FONT_WEIGHT_BOLD, cairo.FONT_WEIGHT_NORMAL]))
        ...
        top_left_x = random.randint(0, int(max_shift_x))
        top_left_y = random.randint(0, int(max_shift_y))
...
class TextImageGenerator(keras.callbacks.Callback):
    ...
    def __init__(self, monogram_file, bigram_file, minibatch_size, img_w,
                 img_h, downsample_width,  num_words, val_split):
    ...
    def build_word_list(self, max_string_len=None, mono_fraction=0.5):
        assert(max_string_len <= TextImageGenerator.ABSOLUTE_MAX_STRING_LEN)
        self.string_list = []
        self.max_string_len = max_string_len
        self.Y_data = np.ones([self.num_words, TextImageGenerator.ABSOLUTE_MAX_STRING_LEN]) * -1
        self.X_text = []
        self.Y_len = [0] * self.num_words

        # monogram file is sorted so just read first N words
        # since some later words are nonsensical
    ...
    def get_batch(self, index, size, train):
        ...
        labels = np.ones([size, TextImageGenerator.ABSOLUTE_MAX_STRING_LEN])
    ...
    def on_epoch_begin(self, epoch, logs={}):
        ...
        if epoch == 10:
            self.build_word_list(8, 1)
        if epoch == 20:
            self.build_word_list(8, 0.5)
        if epoch == 30:
            self.build_word_list(12, 0.5)

# the actual loss calc occurs here despite it not being
# a true Keras loss function
def ctc_lambda_func(inputs):
    # theano doesn't give the last operation a name for some reason,
    # while tensorflow adds ":0" to names..so use a complicated cleanup func
    inputs = {str(i.name).split(":")[0]: i for i in inputs}
    # ..and some package specific code
    ...
    return K.ctc_batch_cost(inputs["the_labels"], y_pred,
                            inputs["input_length"], inputs["label_length"])

# keras still wants a loss func in this particular parameter format
def dummy_func(y_true_dummy, y_pred):
    return y_pred
...
class VizCallback (keras.callbacks.Callback):
    ...
    def __init__(self, test_func, text_img_gen):
        ...
        self.outputDir = datetime.datetime.now().strftime("image_ocr %A, %d. %B %Y %I.%M%p")
    ...
    def show_edit_distance(self, num):
        ...
        mean_ed = mean_ed / num
        print "\nOut of %d samples:  Mean edit distance: %.3f Mean word-length normalized edit distance: %0.3f" \
            % (num, mean_ed, mean_norm_ed)

    def on_epoch_end(self, epoch, logs={}):
        self.model.save_weights(os.path.join(self.outputDir, 'weights%02d.h5' % epoch))
        self.show_edit_distance(256)
        word_batch = next(self.text_img_gen)[0]
        res = decode_batch(self.test_func, word_batch["the_input"][0:VizCallback.num_display_words])

        for i in range(VizCallback.num_display_words):
            pylab.subplot(VizCallback.num_display_words, 1, i + 1)
            pylab.imshow(word_batch["the_input"][i, 0, :, :],  cmap='Greys_r')
            pylab.xlabel("Truth = \'%s\' Decoded = \'%s\'" % (word_batch["source_str"][i], res[i]))
        fig = pylab.gcf()
        fig.set_size_inches(10, 12)
        pylab.savefig(os.path.join(self.outputDir, "e%02d.png" % epoch))
        pylab.close()
...
total_words = 12480
val_split = 0.2
val_words = int(total_words * (val_split))
...
img_gen = TextImageGenerator(monogram_file="wordlist_mono_clean.txt",
                             ...
                             val_split=total_words - val_words)
...
sfm = Activation("softmax", name="softmax")(inner)
Model(input=[input_data], output=[sfm]).summary()

labels = Input(name="the_labels", shape=[img_gen.ABSOLUTE_MAX_STRING_LEN], dtype="float32")
input_length = Input(name="input_length", shape=[1], dtype="int64")
label_length = Input(name="label_length", shape=[1], dtype="int64")
loss_out = merge([sfm, labels, input_length, label_length], mode=ctc_lambda_func, output_shape=(1,), name="ctc")

if K._BACKEND == "tensorflow":  # maybe slight differences in CTC cause this?
    lr = 0.01
    clipnorm = 10
else:
    lr = 0.02
...
sgd = SGD(lr=lr, decay=1e-6, momentum=0.9, nesterov=True, clipnorm=clipnorm)

model = Model(input=[input_data, labels, input_length, label_length], output=[loss_out])

model.compile(loss={"ctc": dummy_func}, optimizer=sgd)

# captures output of softmax so we can decode the output during visualization
test_func = K.function([input_data], [sfm])

viz_cb = VizCallback(test_func, img_gen.next_val())

model.fit_generator(generator=img_gen.next_train(), samples_per_epoch=(total_words - val_words),
