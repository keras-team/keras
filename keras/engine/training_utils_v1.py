...
def _append_composite_tensor(target, to_append):
  ...
  if isinstance(target, tf.SparseTensor):
    ...
    # won't work.
    return tf.compat.v1.sparse.concat(sp_inputs=[target, to_append], axis=0)
  ...
  if isinstance(class_weight, dict):
    ...
    if tf.is_tensor(y):
      ...
      y_classes = smart_cond.smart_cond(
          ...
          lambda: tf.cast(K.reshape(y, (-1,)), tf.dtypes.int64))
      ...
      class_sample_weight = tf.cast(class_sample_weight, K.floatx())
      if sample_weight is not None:
        sample_weight = tf.cast(
            tf.compat.v2.convert_to_tensor(sample_weight), K.floatx())
    else:
      y_classes = y
      if len(y.shape) == 2:
        if y.shape[1] > 1:
...
def infer_steps_for_dataset(model,
  ...
  size = K.get_value(tf.data.experimental.cardinality(dataset))
