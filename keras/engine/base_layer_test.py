...
class DynamicLayer(base_layer.Layer):
  ...
  def call(self, inputs):
    ...
    for idx, sample in enumerate(inputs):
      samples = samples.write(idx, tf.math.square(sample))
...
class BaseLayerTest(keras_parameterized.TestCase):
  ...
  def test_manual_compute_output_shape(self):
    ...
    layer = BuildCounter(dtype=tf.dtypes.float64)
    ...
    output_signature = layer.compute_output_signature(
        tf.TensorSpec(dtype=tf.dtypes.float64, shape=[None, 10]))
    self.assertEqual(layer.build_counter, 1)
    self.assertEqual(layer.build_shape.as_list(), [None, 10])
    self.assertEqual(output_signature.dtype, tf.dtypes.float64)
  ...
  def test_deepcopy(self):
    bias_reg = lambda x: 1e-3 * tf.compat.v2.math.reduce_sum(x)
...
class SymbolicSupportTest(keras_parameterized.TestCase):
  ...
  def test_summaries_in_tf_function(self):
    ...
    class MyLayer(base_layer.Layer):
      ...
      def call(self, inputs):
        tf.compat.v2.summary.scalar('mean', tf.compat.v2.math.reduce_mean(inputs))
        return inputs
...
class NestedTrackingTest(tf.test.TestCase):
  ...
  def test_nested_layer_updates_losses_tracking(self):
    ...
    class UpdateAndLossLayer(base_layer.Layer):
      ...
      def call(self, inputs):
        self.add_loss(tf.compat.v2.math.reduce_sum(inputs))
...
class AutographControlFlowTest(keras_parameterized.TestCase):
  ...
  def test_if_training_pattern_loss(self):
    ...
    class MyLayer(base_layer.Layer):
      ...
      def call(self, inputs, training=None):
        if training:
          loss = tf.compat.v2.math.reduce_sum(inputs)
        else:
          loss = 0.
        self.add_loss(loss)
        return inputs
  ...
  def test_if_training_pattern_metric(self):
    ...
    class MyLayer(base_layer.Layer):
      ...
      def call(self, inputs, training=None):
        if training:
          metric = tf.compat.v2.math.reduce_sum(inputs)
        else:
          metric = 0.
  ...
  def test_conditional_callable_losses(self):
    ...
    def get_losses(t):
      if t < 0:
        return tf.compat.v2.math.reduce_sum(model.losses) * t
      else:
        return tf.compat.v2.math.reduce_sum(model.losses)
  ...
  def test_conditional_metrics_in_call(self):
    ...
    class MyLayer(base_layer.Layer):
      ...
      def call(self, inputs, training=None):
        if training:
          self.add_metric(tf.compat.v2.math.reduce_sum(inputs),
                          name='sum',
                          aggregation='mean')
        return inputs
  ...
  def test_conditional_activity_regularizer_in_call(self):
    ...
    class TestModel(training_lib.Model):
      ...
      def call(self, x, training=None):
        if tf.greater(tf.compat.v2.math.reduce_sum(x), 0.0):
          return self.layer(x)
        else:
          return self.layer(x)
...
class DTypeTest(keras_parameterized.TestCase):
  ...
  def test_passing_dtype_to_constructor(self):
    ...
    layer = IdentityLayer(dtype=tf.dtypes.float64)
    layer(self._const('float32'))
    self.assertEqual(layer.dtype, 'float64')
