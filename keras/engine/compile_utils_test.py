...
class LossesContainerTest(keras_parameterized.TestCase):
  ...
  def test_loss_masking(self):
    ...
    y_p = tf.compat.v2.constant([[[1], [1]], [[0], [0]]], dtype=tf.float32)
    y_t = tf.compat.v2.constant([[[1], [1]], [[1], [1]]], dtype=tf.float32)
  ...
  def test_loss_sample_weight(self):
    ...
    y_p = tf.compat.v2.constant([[[1], [1]], [[0], [0]]], dtype=tf.float32)
    y_t = tf.compat.v2.constant([[[1], [1]], [[1], [1]]], dtype=tf.float32)
    sw = tf.compat.v2.constant([[.2, .3], [.5, 0]], dtype=tf.float32)
...
class MetricsContainerTest(keras_parameterized.TestCase):
  ...
  def test_metrics_sample_weight(self):
    metrics_container = compile_utils.MetricsContainer(
        metrics=['mae'], weighted_metrics=['mse'])
    y_p = tf.compat.v2.constant([[[1], [1]], [[0], [1]]], dtype=tf.float32)
    y_t = tf.compat.v2.constant([[[1], [1]], [[1], [1]]], dtype=tf.float32)
    sw = tf.compat.v2.constant([[.2, .3], [.5, 0]], dtype=tf.float32)
  ...
  def test_metrics_masking_sample_weight(self):
    ...
    y_p = tf.compat.v2.constant([[[1], [1]], [[0], [1]]], dtype=tf.float32)
    y_t = tf.compat.v2.constant([[[1], [1]], [[1], [1]]], dtype=tf.float32)
    sw = tf.compat.v2.constant([[.3, .2], [.2, .3]], dtype=tf.float32)
    y_p._keras_mask = tf.compat.v2.constant([[1, 0], [1, 0]],
                                           dtype=tf.float32)
