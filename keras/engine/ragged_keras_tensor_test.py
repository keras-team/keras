...
class RaggedTensorClassMethodAsLayerTest(keras_parameterized.TestCase):
  ...
  def test_from_row_limits(self):
    row_limits = tf.compat.v2.constant([2, 2, 5, 6, 7], tf.int64)
  ...
  def test_from_nested_value_row_ids(self):
    nested_value_rowids = [
        tf.compat.v2.constant([0, 0, 1, 3, 3], tf.int64),
        tf.compat.v2.constant([0, 0, 2, 2, 2, 3, 4], tf.int64)
    ]
    inp = layers.Input(shape=[None], dtype=tf.string)
  ...
  def test_from_nested_row_splits(self):
    nested_row_splits = [
        tf.compat.v2.constant([0, 2, 3, 3, 5], tf.int64),
        tf.compat.v2.constant([0, 2, 2, 5, 6, 7], tf.int64)
    ]
    inp = layers.Input(shape=[None], dtype=tf.string)
  ...
  def test_from_nested_row_lengths(self):
    nested_row_lengths = [
        tf.compat.v2.constant([2, 1, 0, 2], tf.int64),
        tf.compat.v2.constant([2, 0, 3, 1, 1], tf.int64)
    ]
    inp = layers.Input(shape=[None], dtype=tf.string)
  ...
  def test_from_sparse(self):
    ...
    shape = [4, 5]
    sp_value = tf.sparse.SparseTensor(indices, values, shape)
