...
BASE_WEIGHTS_PATH = (
    "https://huggingface.co/anasrz/kimm/resolve/main"
)

WEIGHTS_NAMES = {
    "mixer_l16_224": (
        "jx_mixer_l16_224-92f9adc4.weights.h5"
    ),
    "mixer_b16_224": (
        "mixer_b16_224_miil-9229a591.weights.h5?download=true"
    )
}
...
def MlpMixer(num_classes=1000,
    ...
    assert keras.config.image_data_format() == 'channels_last', "Mixer only supports `channels_last` data_format"
...
def get_weights_pretrained(model, model_name):
    ...
    assert model_name in WEIGHTS_NAMES.keys(), f"Weights not available for model {model_name}"
