...
class WAME(Optimizer):
    '''WAME optimizer, as described in
    ...
    # Arguments
        ...
        eta_plus/eta_minus: floats for the adaptation of delta
        eta_min/eta_max: bounds for eta_minus and eta_plus respectively
        beta_a: beta for acceleration factor
...
class RPROP(Optimizer):
    ...
    '''
    def __init__(self, lr=0.001, decay=0., eta_plus = 1.2, eta_minus = 0.1,
                 eta_min=1e-2, eta_max=1e2, **kwargs):
    ...
    def get_updates(self, params, constraints, loss):
        ...
        prev_grads = [K.zeros(shape) for shape in shapes]
        prev_param = [K.zeros(shape) for shape in shapes]
        alphas = [K.ones(shape) for shape in shapes]

        for p, g, a, pg, pp in zip(params, grads, alphas, prev_grads, prev_param):
            change = pg * g
            change_above_zero = K.greater(change,0.)
            change_below_zero = K.less(change,0.)
            ...
            self.updates.append(K.update(a, a_clipped))
            self.updates.append(K.update(pp, p))
        return self.updates
