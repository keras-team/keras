...
def dot(x, y):
    ...
    if ndim(x) is not None and (ndim(x) > 2 or ndim(y) > 2):
        x_shape = tuple([s if i is None else i for i, s in zip(int_shape(x), tf.unpack(tf.shape(x)))])
        y_shape = tuple([s if i is None else i for i, s in zip(int_shape(y), tf.unpack(tf.shape(y)))])
