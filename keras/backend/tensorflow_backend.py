...
def _is_current_explicit_device(device_type):
    ...
    # Returns
        A boolean indicating if the current device scope is explicitly
        set on the device type.
...
def _has_nchw_support():
    ...
    Therefore we check if we are not explicitly put on CPU, and have GPUs available.
    ...
    if kwargs:
        for key in kwargs:
            if not (has_arg(tf.Session.run, key, True) or
                ...
                msg = 'Invalid argument "%s" passed to ' \
                      'K.function with TensorFlow backend' % key
    ...
    if unroll:
        ...
        if mask is not None:
            ...
            def _step(time, output_ta_t, *states):
                ...
                new_states = [
                    tf.where(
                        tf.tile(mask_t, tf.stack([1, tf.shape(new_states[i])[1]])),
...
def ctc_label_dense_to_sparse(labels, label_lengths):
    ...
    return tf.SparseTensor(tf.to_int64(indices),
                           vals_sparse, tf.to_int64(label_shape))
...
def ctc_decode(y_pred, input_length, greedy=True, beam_width=100,
    ...
    decoded_dense = [tf.sparse_to_dense(st.indices, st.dense_shape, st.values,
                                        default_value=-1)
                     for st in decoded]
...
def local_conv2d(inputs, kernel, kernel_size,
                 strides, output_shape, data_format=None):
