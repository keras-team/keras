...
_LEARNING_PHASE_PLACEHOLDER = C.constant(shape=(), dtype=np.float32, value=1.0,\
...
def eval(x):
    if isinstance(x, C.cntk_py.Function):
        return x.eval()
    elif isinstance(x, C.variables.Constant) or isinstance(x, \
                                                        C.variables.Parameter):
        return x.value
    else:
...
def random_normal(shape, mean=0.0, stddev=1.0, dtype=None, seed=None):
    ...
    if seed is None:
        # ensure that randomness is conditioned by the Numpy RNG
        seed = np.random.randint(10e3)
    return C.random.normal(shape=shape, mean=mean,\
...
def squeeze(x, axis):
    ...
    new_shape = shape[nones:]
    new_shape = tuple([C.InferredDimension if _ == C.FreeDimension \
...
def reshape(x, shape):
    shape = tuple([C.InferredDimension if _ == C.FreeDimension \
                                        else _ for _ in shape])
...
def permute_dimensions(x, pattern):
    ...
    if num_dynamic_axis > 0 and pattern[:num_dynamic_axis] != \
                                                current_layout[:num_dynamic_axis]:
...
def rnn(step_function, inputs, initial_states,
        ...
        for constant in constants:
            if isinstance(constant, list):
                ...
                if _get_dynamic_axis_num(constant) == 1:
                    rnn_constants.append(C.sequence.broadcast_as(constant, \
                                                                rnn_inputs))
                else:
    ...
    with C.default_options(axis_offset=1):
        def _recurrence(x, states, m):
            ...
            if m is not None:
                new_states = [C.element_select(m, n, s) \
    ...
    for l_s, i_s in zip(last_states, initial_states):
        if _get_dynamic_axis_num(i_s) == 0 and _get_dynamic_axis_num(l_s) == 1:
            if hasattr(C, 'unpack_batch'):
                ...
                f_stats.append(C.user_function(\
                                    ConvertToStatic(l_s, batch_size=i_s.shape[0])))
        else:
            f_stats.append(l_s)
    ...
    def __call__(self, inputs):
        ...
        if self.metrics_func is not None:
            ...
            if (self.unrelated_updates is None and
                    (_LEARNING_PHASE_PLACEHOLDER.value == 1.0 \
                            or _LEARNING_PHASE == 1)):
