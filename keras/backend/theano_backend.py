...
def batch_dot(x, y, axes=None):
    ...
    # Returns
        A tensor with shape equal to the concatenation of x’s shape (less the dimension that was summed over) and y’s shape (less the batch dimension and the dimension that was summed over). If final rank is 1, we reshape it to (batch_size, 1).
    '''
    if type(axes) == int:
        axes = (axes, axes)
    if axes is None:
