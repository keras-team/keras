  ...
  def testFunctionalConv2DInitializerFromScope(self):
    with tf.Graph().as_default(), self.cached_session():
      with tf.compat.v1.variable_scope(
          'scope', initializer=tf.compat.v1.initializers.ones()):
        height, width = 7, 9
        images = tf.random.uniform((5, height, width, 3), seed=1)
        conv_layers.conv2d(images, 32, [3, 3], name='conv1')
...
class SeparableConv1DTest(tf.test.TestCase):
  ...
  def testConstraints(self):
    d_constraint = lambda x: x / tf.compat.v2.math.reduce_sum(x)
    p_constraint = lambda x: x / tf.compat.v2.math.reduce_sum(x)
    b_constraint = lambda x: x / tf.compat.v2.math.reduce_max(x)
