...
tf.compat.v2.__internal__.register_clear_session_function(clear_session)
...
# from TFLite to Keras.
tf.compat.v2.__internal__.register_get_session_function(get_session)

# Inject the get_session function to tracking_util to avoid the backward
# dependency from TF to Keras.
tf.compat.v2.__internal__.tracking.register_session_provider(get_session)
...
def is_sparse(tensor):
  ...
  return isinstance(tensor, tf.sparse.SparseTensor)
...
def is_keras_tensor(x):
  ...
  if not isinstance(x,
                    (tf.Tensor, tf.compat.v2.Variable,
                     tf.sparse.SparseTensor, tf.RaggedTensor,
...
def eye(size, dtype=None, name=None):
  ...
  if dtype is None:
    dtype = floatx()
  tf_dtype = tf.as_dtype(dtype)
  return variable(tf.linalg.eye(size, dtype=tf_dtype), dtype, name)
...
def moving_average_update(x, value, momentum):
  ...
  if tf.compat.v2.__internal__.tf2.enabled():
    ...
    return tf.compat.v2.__internal__.train.assign_moving_average(
...
def max(x, axis=None, keepdims=False):
  ...
  """
  return tf.compat.v2.reduce_max(x, axis, keepdims)
...
def min(x, axis=None, keepdims=False):
  ...
  """
  return tf.compat.v2.reduce_min(x, axis, keepdims)
...
def sum(x, axis=None, keepdims=False):
  ...
  """
  return tf.compat.v2.reduce_sum(x, axis, keepdims)
...
def prod(x, axis=None, keepdims=False):
  ...
  """
  return tf.compat.v2.reduce_prod(x, axis, keepdims)
...
def mean(x, axis=None, keepdims=False):
  ...
  if x.dtype.base_dtype == tf.bool:
    x = tf.cast(x, floatx())
  return tf.compat.v2.reduce_mean(x, axis, keepdims)
...
def any(x, axis=None, keepdims=False):
  ...
  """
  x = tf.cast(x, tf.bool)
  return tf.compat.v2.reduce_any(x, axis, keepdims)
...
def all(x, axis=None, keepdims=False):
  ...
  """
  x = tf.cast(x, tf.bool)
  return tf.compat.v2.reduce_all(x, axis, keepdims)
...
def argmax(x, axis=-1):
  ...
  Returns:
      A tensor.
  """
  return tf.compat.v1.math.argmax(x, axis)
...
def argmin(x, axis=-1):
  ...
  Returns:
      A tensor.
  """
  return tf.compat.v1.math.argmin(x, axis)
...
def logsumexp(x, axis=None, keepdims=False):
  ...
  Returns:
      The reduced tensor.
  """
  return tf.compat.v2.reduce_logsumexp(x, axis, keepdims)
...
def resize_images(x, height_factor, width_factor, data_format,
  ...
  if new_shape.is_fully_defined():
    new_shape = tf.compat.v2.constant(new_shape.as_list(), dtype='int32')
  else:
    new_shape = tf.compat.v2.shape(x)[rows:cols + 1]
...
def rnn(step_function,
  ...
  if unroll:
    ...
    if (not tf.compat.v2.executing_eagerly() and
      ...
      max_iterations = tf.compat.v2.reduce_max(input_length)
    else:
      max_iterations = None
    ...
    if mask is not None:
      ...
      if go_backwards:
        max_len = tf.compat.v2.reduce_max(input_length, axis=0)
        rev_input_length = tf.subtract(max_len - 1, input_length)

        def masking_fn(time):
          return tf.math.less(rev_input_length, time)
      else:

        def masking_fn(time):
          return tf.math.greater(input_length, time)
...
def relu(x, alpha=0., max_value=None, threshold=0):
  ...
  if threshold != 0:
    ...
    x = x * tf.cast(tf.math.greater(x, threshold), dtype=dtype)
  elif max_value == 6:
...
def softmax(x, axis=-1):
  ...
  Returns:
      A tensor.
  """
  return tf.compat.v1.nn.softmax(x, axis=axis)
...
def categorical_crossentropy(target, output, from_logits=False, axis=-1):
  ...
  output = output / tf.compat.v2.reduce_sum(output, axis, True)
  # Compute cross entropy from probabilities.
  epsilon_ = _constant_to_tensor(epsilon(), output.dtype.base_dtype)
  output = tf.clip_by_value(output, epsilon_, 1. - epsilon_)
  return -tf.compat.v2.reduce_sum(target * tf.math.log(output), axis)
...
def in_top_k(predictions, targets, k):
  ...
  """
  return tf.compat.v1.nn.in_top_k(predictions, targets, k)
...
def ctc_label_dense_to_sparse(labels, label_lengths):
  ...
  vals_sparse = tf.compat.v1.manip.gather_nd(labels, indices)
