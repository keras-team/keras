...
class TpuStrategyTest(tf.test.TestCase):
  ...
  def define_kpls_for_training(self, use_adapt):
    if use_adapt:
      feature_lookup_layer = tf.keras.layers.experimental.preprocessing.StringLookup(
          num_oov_indices=1)
      feature_lookup_layer.adapt(FEATURE_VOCAB)
      label_lookup_layer = tf.keras.layers.experimental.preprocessing.StringLookup(
