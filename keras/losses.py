...
def _ragged_tensor_apply_loss(loss_fn, y_true, y_pred):
  ...
  def rt_is_equiv_dense(rt):
    ...
    Args:
      rt: RaggedTensor
    """
    return tf.compat.v2.math.reduce_all([
        tf.equal(
...
def _maybe_convert_labels(y_true):
  ...
  is_binary = tf.compat.v2.math.reduce_all(tf.logical_or(are_zeros, are_ones))
...
def squared_hinge(y_true, y_pred):
  ...
  return K.mean(
      tf.math.square(tf.math.maximum(1. - y_true * y_pred, 0.)), axis=-1)
...
def hinge(y_true, y_pred):
  ...
  y_true = tf.cast(y_true, y_pred.dtype)
  y_true = _maybe_convert_labels(y_true)
  return K.mean(tf.math.maximum(1. - y_true * y_pred, 0.), axis=-1)
...
def categorical_hinge(y_true, y_pred):
  ...
  y_pred = tf.compat.v2.convert_to_tensor(y_pred)
  y_true = tf.cast(y_true, y_pred.dtype)
  pos = tf.compat.v2.math.reduce_sum(y_true * y_pred, axis=-1)
  neg = tf.compat.v2.math.reduce_max((1. - y_true) * y_pred, axis=-1)
  zero = tf.cast(0., y_pred.dtype)
  return tf.math.maximum(neg - pos + 1., zero)
...
def huber(y_true, y_pred, delta=1.0):
  ...
  return K.mean(
      tf.compat.v2.where(
          abs_error <= delta, half * tf.math.pow(error, 2),
          half * tf.math.pow(delta, 2) + delta * (abs_error - delta)),
      axis=-1)
...
def kl_divergence(y_true, y_pred):
  ...
  return tf.compat.v2.math.reduce_sum(y_true * tf.math.log(y_true / y_pred), axis=-1)
