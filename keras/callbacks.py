...
def keras_model_summary(name, data, step=None):
  ...
  with tf.compat.v2.summary.experimental.summary_scope(name, 'graph_keras_model',
    ...
    with tf.compat.v1.device('cpu:0'):
      tensor = tf.compat.v2.constant(json_string, dtype=tf.string)
...
class TensorBoard(Callback, version_utils.TensorBoardVersionSelector):
  ...
  * Sampled profiling
  ...
  def _push_writer(self, writer, step):
    ...
    should_record = lambda: tf.math.equal(step % self.update_freq, 0)
  ...
  def on_test_end(self, logs=None):
    if self.model.optimizer and hasattr(self.model.optimizer, 'iterations'):
      with tf.compat.v2.summary.record_if(True), self._val_writer.as_default():
  ...
  def _start_profiler(self, logdir):
    ...
    if self._profiler_started:
      return
    try:
      tf.compat.v2.profiler.experimental.start(logdir=logdir)
  ...
  def _stop_profiler(self, save=True):
    ...
    if not self._profiler_started:
      return
    try:
      tf.compat.v2.profiler.experimental.stop(save=save)
