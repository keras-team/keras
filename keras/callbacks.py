...
@keras_export("keras.callbacks.BestModelExporter")
class BestModelExporter(ModelCheckpoint):
    ...
    When using the BestModelExporter callback in conjunction with
    `keras.utils.SidecarEvaluator`, users can provide the`filepath` and
    `ckpt_filepath`.
    The callback will then export the model that the evaluator deems as the best
    ...
    sidecar_evaluator = keras.utils.SidecarEvaluator(
        ...
        callbacks=[
            ModelCheckpoint(
                ...
                ckpt_filepath=os.path.join(checkpoint_dir, 'ckpt-{epoch:04d}'),
                save_freq="eval",
                save_weights_only=True,
                monitor="loss",
                mode="min",
    ...
    Args:
        filepath: Path where best models should be saved by this
          `BestModelExporter`callback. Epoch formatting options, such as
          `os.path.join(best_model_dir, 'best-model-{epoch:04d}')`, can be used
          to allow saved model to preserve epoch information in the file name.
          BestModelExporter will use the "training epoch" at which the
          checkpoint was saved by training to fill the epoch placeholder in the
          path.
        ckpt_filepath: Path where checkpoints are saved by training. This should
          be the same as what is provided to `filepath` argument of
          `ModelCheckpoint` on the training side, such as
          `os.path.join(checkpoint_dir, 'ckpt-{epoch:04d}')`.
    """

    def __init__(self, filepath, ckpt_filepath, **kwargs):
        super().__init__(
            filepath=filepath,
            save_best_only=True,
            **kwargs,
        ...
        self._ckpt_filepath = ckpt_filepath
