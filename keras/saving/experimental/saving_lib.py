...
def save_model(model, filepath):
    ...
    try:
        with open(os.path.join(temp_path, _METADATA_FILENAME), "w") as f:
            f.write(metadata_json)
        with open(os.path.join(temp_path, _CONFIG_FILENAME), "w") as f:
            f.write(config_json)
        _save_state(model, os.path.join(temp_path, _STATES_ROOT_DIRNAME), set())
...
def load_model(filepath, custom_objects=None):
    ...
    try:
        ...
        with open(os.path.join(temp_path, _CONFIG_FILENAME), "r") as f:
            config_json = f.read()
        ...
        _load_state(model, os.path.join(temp_path, _STATES_ROOT_DIRNAME))
    except Exception as e:
        raise e
    else:
        return model
