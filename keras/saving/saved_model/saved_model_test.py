...
class TestSavedModelFormatAllModes(keras_parameterized.TestCase):
  ...
  def _get_model(self):
    ...
    model.compile(
        ...
        optimizer='rmsprop')
    def callable_loss():
      return tf.compat.v2.reduce_sum(model.weights[0])
  ...
  def test_trainable_weights(self):
    ...
    layer.add_weight(
        'extra_weight', shape=[],
        initializer=tf.compat.v1.initializers.constant(11),
        trainable=True)
    layer.add_weight(
        'extra_weight_2', shape=[],
        initializer=tf.compat.v1.initializers.constant(12),
        trainable=False)
    ...
    class EmbeddingMerger(keras.layers.Layer):
      ...
      def __init__(self, list_features, **kwargs):
        ...
        self.mean = keras.layers.Lambda(
            tf.compat.v2.reduce_mean, arguments=dict(axis=1))
  ...
  def testSaveLayerMultipleInputs(self):
    ...
    model = CustomModel()
    inp = [tf.compat.v2.constant(i, shape=[1, 1], dtype=tf.dtypes.float32)
           for i in range(1, 5)]
    expected = model(inp)
  ...
  def test_wrapped_layer_training(self):
    class Custom(keras.models.Model):
      ...
      def call(self, inputs):
        return self.layer(inputs)
    model = Custom()
    x = tf.compat.v2.constant(1., shape=[1, 1])
    ...
    assert_loaded_model(tf.compat.v2.saved_model.load(saved_model_dir))
