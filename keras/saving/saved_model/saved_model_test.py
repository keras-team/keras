...
class LayerWithLoss(keras.layers.Layer):
  ...
  def call(self, inputs):
    self.add_loss(tf.compat.v2.math.reduce_sum(inputs), inputs=inputs)
    return inputs * 2
...
class TestSavedModelFormatAllModes(keras_parameterized.TestCase):
  ...
  def _test_save_and_load(self, use_dataset=False):
    ...
    model.compile(
        ...
        optimizer='rmsprop')
    def callable_loss():
      return tf.compat.v2.math.reduce_sum(model.weights[0])
    ...
    self.evaluate(tf.compat.v1.initializers.variables(loaded.variables))
  ...
  def test_trainable_weights(self):
    ...
    saved_model_dir = self._save_model_dir()
    self.evaluate(tf.compat.v1.initializers.variables(layer.variables))
  ...
  def test_layer_with_learning_phase(self):
    ...
    self.assertAllEqual(
        input_arr, loaded(input_arr, training=tf.compat.v2.constant(False)))
    self.assertAllEqual(
        tf.zeros((4, 3)),
        loaded(input_arr, training=tf.compat.v2.constant(True)))
  ...
  def test_standard_loader(self):
    ...
    loaded = tf.compat.v2.saved_model.load(saved_model_dir)
    self.evaluate(tf.compat.v1.initializers.variables(loaded.variables))
  ...
  def testBatchNormUpdates(self):
    ...
    input_arr = tf.compat.v2.constant([[11], [12], [13]], dtype=tf.dtypes.float32)
    input_arr2 = tf.compat.v2.constant([[14], [15], [16]], dtype=tf.dtypes.float32)
  ...
  def testSaveWithSignatures(self):
    ...
    feature_configs = {
        'inputs': tf.io.FixedLenFeature(
            shape=[2, 3], dtype=tf.dtypes.float32)}

    @tf.function(
        input_signature=[tf.TensorSpec([None], tf.dtypes.string)])
    def parse_and_predict(examples):
      features = tf.compat.v1.io.parse_single_example(examples[0], feature_configs)
    ...
    self.assertAllClose(
        ...
        loaded.signatures['predict'](tf.compat.v2.convert_to_tensor(
    ...
    outputs = loaded.signatures['parse_and_predict'](
        tf.compat.v2.convert_to_tensor([example.SerializeToString()]))
  ...
  def testTrainingDefaults(self):
    ...
    saved_model_dir = self._save_model_dir()
    model.save(saved_model_dir, save_format='tf')
    load = tf.compat.v2.saved_model.load(saved_model_dir)
  ...
  def testModelWithTfFunctionCall(self):
    class Subclass(keras.models.Model):
      ...
      def call(self, inputs, training=False):
        return inputs * tf.cast(training, tf.dtypes.float32)
    ...
    self.assertAllEqual(
        [[1, 5]],
        self.evaluate(loaded(tf.compat.v2.constant([[1, 5.]]), training=True)))
    self.assertAllEqual(
        [[0, 0]],
        self.evaluate(loaded(tf.compat.v2.constant([[1, 5.]]), training=False)))
  ...
  def testSaveInStrategyScope(self):
    self._testAddUpdate(tf.compat.v2.distribute.MirroredStrategy().scope())
    ...
    class EmbeddingMerger(keras.layers.Layer):
      ...
      def __init__(self, list_features, **kwargs):
        ...
        self.mean = keras.layers.Lambda(
            tf.compat.v2.math.reduce_mean, arguments=dict(axis=1))
...
class TestSavedModelFormat(tf.test.TestCase):
  ...
  def test_save_without_tracing(self):
    ...
    with self.assertRaisesRegex(ValueError, 'Cannot call custom layer'):
      loaded.attached_layer(tf.compat.v2.constant([1.]))
...
class MetricTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_custom_metric_wrapped_call(self):
    ...
    class NegativeMean(keras.metrics.Mean):
      ...
      @tf.function(
          input_signature=[tf.TensorSpec(None, tf.dtypes.float32)])
