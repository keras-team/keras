...
class L1L2(Regularizer):
  ...
  def __call__(self, x):
    ...
    if self.l1:
      regularization += self.l1 * tf.compat.v2.math.reduce_sum(tf.abs(x))
    if self.l2:
      regularization += self.l2 * tf.compat.v2.math.reduce_sum(tf.math.square(x))
...
class L1(Regularizer):
  ...
  def __call__(self, x):
    return self.l1 * tf.compat.v2.math.reduce_sum(tf.abs(x))
...
class L2(Regularizer):
  ...
  def __call__(self, x):
    return self.l2 * tf.compat.v2.math.reduce_sum(tf.math.square(x))
