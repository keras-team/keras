...
class HSVToRGB(Operation):
    ...
    def compute_output_spec(self, images):
        ...
        channels_axis = -1 if self.data_format == "channels_last" else -3
        channels = images_shape[channels_axis]
        # See the identical comment in RGBToHSV.compute_output_spec above:
        # a `None` channel count here can mean either a genuinely dynamic
        # channel dim (do not raise) or an under-specified `keras.Input`
        # shape whose implicit batch dim happens to land on this axis
        # (not detectable from shape alone).
        if channels is not None and channels != 3:
            raise ValueError(
                "Input images must have 3 channels, but received images with "
                f"{channels} channels."
