...
class MathDtypeTest(testing.TestCase):
    ...
    def test_gammaincc(self, dtypes):
        import jax.numpy as jnp
        import jax.scipy.special
        ...
        expected_dtype = standardize_dtype(
            jax.scipy.special.gammaincc(x1_jax, x2_jax).dtype
        )

        self.assertEqual(
            standardize_dtype(kmath.gammaincc(x1, x2).dtype),
            expected_dtype,
        )
        self.assertEqual(
            standardize_dtype(kmath.Gammaincc().symbolic_call(x1, x2).dtype),
            expected_dtype,
        )
