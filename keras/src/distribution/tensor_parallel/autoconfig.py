...
def analyze_dense_layer(layer):
    """Analyzes a Keras Dense layer to classify its sharding strategy based on dimension change.
...
def _recursive_layer_traversal(
    ...
    """Recursively traverses the model graph to apply sharding rules for tensor parallelism.
    ...
    rules based on layer type and heuristic classification (e.g., up/down projection).

    - Split Logic:
        - 'up_projection' (or general 'dense'): Column-wise sharding (`Split(..., 1, "column")`) 
            on kernel. Requires output to be gathered (`gather`).
        - 'down_projection' (or attention output): Row-wise sharding (`Split(..., 0, "row")`) 
            on kernel. Requires output to be reduced (`allreduce`).
    ...
    Args:
        ...
        state_rules: A dictionary to accumulate variable sharding rules (`LayoutMap.state_rules`).
        output_rules: A dictionary to accumulate layer output communication rules 
        (`LayoutMap.output_rules`).
        processed_layers: A set of layer IDs to prevent infinite recursion in graph structures.
    """ 
    ...
    if isinstance(current_layer, layers.Dense):
        ...
        if mlp_type == 'up_projection':
            # Column-wise sharding for the first MLP layer (key/query/value/up-projection)
            ...
            # The result from different devices needs to be summed (all-reduced).
...
def get_default_config_keras(module, device_ids):
    """Generates a default tensor parallelism sharding configuration for a Keras model.
    ...
    Args:
        module: The root Keras `Model` or `Layer` instance representing the module
                to be sharded.
        device_ids: A list of device identifiers (e.g., strings) that define the
                    parallelism group. The length of this list determines the
                    number of slices (`device_count`).
