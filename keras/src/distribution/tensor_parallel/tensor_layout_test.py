...
from keras.src.distribution.tensor_parallel.tensor_layout import LayoutAction, Split, LayoutMap
...
class LayoutTest(testing.TestCase):
    ...
    def test_layout_action_abstract_methods_raise_error(self):
        """Ensures the base class methods raise NotImplementedError as expected."""
        action = LayoutAction()
    ...
    def test_split_and_undo_cycle_even(self):
        """Tests the full cycle of splitting and then reconstructing an evenly divisible tensor."""
        world_size = 2
    ...
    def test_split_with_sharding_type_hint(self):
        ...
        action_col = Split(world_size=world_size, dim=-1, sharding_type="column")
    ...
    def test_layout_map_initialization_and_methods(self):
        """Tests basic initialization and method behavior of the LayoutMap class."""
        ...
        # Verify that create_collective_ops is chainable (returns self)
        self.assertIs(layout_map.create_collective_ops(devices=["cpu:0"]), layout_map)