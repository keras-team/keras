...
def run_model_verification(preset_name, model_class):
    ...
    Returns a dictionary with history and memory profiling results.
    """
    print(f"🔧 TRAINING AND PROFILING FOR: {preset_name.upper()}")
    print("=" * 60)
    ...
    if num_devices < 2:
        print(f"  ⚠️ Skipping test for {preset_name}: requires at least 2 JAX devices, but found {num_devices}.")
        return {"status": "SKIPPED", "history": None, "memory": None}

    # --- 2. Baseline Memory Measurement ---
    print("\n  --- Measuring Baseline (Unsharded) Memory ---")
    
    # Capture memory BEFORE loading the model
    mem_info_before = PROCESS.memory_info()
    base_rss = mem_info_before.rss
    log_memory_usage("Start of Baseline")

    baseline_model = get_model_from_preset(preset_name, model_class)
    # Move weights to a device to ensure memory is properly accounted for
    _ = jax.device_put(baseline_model.weights, jax.devices()[0])

    # Capture memory AFTER loading the model
    mem_info_after = PROCESS.memory_info()
    replicated_rss = mem_info_after.rss
    log_memory_usage("After loading baseline model on Device 0")

    # --- 3. Calculate Memory Costs ---
    replicated_model_cost_mb = (replicated_rss - base_rss) / 1024 / 1024
    # Theoretical memory per device after sharding
    sharded_model_cost_mb = replicated_model_cost_mb / num_devices
    
    memory_results = {
        "replicated_mb": replicated_model_cost_mb,
        "sharded_mb": sharded_model_cost_mb,
    }

    # Clean up to free memory before the next step
    del baseline_model
    import gc
    gc.collect()
    log_memory_usage("After cleaning up baseline model")

    # --- 4. Sharded Model Training ---
    ...
    log_memory_usage("Before Parallax Sharding")
    with distribution.scope():
        distribution.shard(parallax_model, sample_batch_np)
    print("    ✅ Auto-sharding plan applied.")
    log_memory_usage("After Parallax Sharding on 2 Devices")
    ...
    return {
        "status": "COMPLETED",
        "history": epoch_by_epoch_history,
...
if __name__ == "__main__":
    ...
    for preset, model_class in MODEL_MAPPING.items():
        try:
            run_data = run_model_verification(preset, model_class)
            results[preset] = run_data
    ...
    # --- Final Memory Summary Report ---
    print("\n" + "=" * 70)
    print("📊 MEMORY PROFILING SUMMARY")
    
    for preset, data in results.items():
        if data["status"] == "COMPLETED" and data["memory"]:
            print("\n" + f"--- Model: {preset} ---")
            
            rep_mem = data["memory"]["replicated_mb"]
            shard_mem = data["memory"]["sharded_mb"]
            
            if rep_mem > 0:
                savings = 100 * (1 - (shard_mem / rep_mem))
            else:
                savings = 0.0

            print("---------------------- Memory Usage Results ----------------------")
            print(f"    - Replicated (Full Model) : {rep_mem:9.2f} MB")
            print(f"    - Sharded (Per Device)    : {shard_mem:9.2f} MB")
            print(f"    - Memory Savings          : {savings:9.2f}%")
            print("----------------------------------------------------------------")
    ...
    with open(results_filename, "w") as f:
        # Custom serializer to handle numpy types if they appear
    ...
    print(f"\n✅ Training summary and detailed metrics saved to {results_filename}")