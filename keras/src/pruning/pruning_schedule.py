...
class PolynomialDecay(PruningSchedule):
    ...
    def __init__(self, initial_sparsity=0.0, target_sparsity=0.8, power=3.0, 
                 start_step=0, end_step=1000, frequency=100):
        super().__init__(start_step, end_step, frequency)
        
        if not 0 <= initial_sparsity <= 1:
            raise ValueError(f"initial_sparsity must be between 0 and 1. Got: {initial_sparsity}")
        ...
        if initial_sparsity >= target_sparsity:
            raise ValueError(f"initial_sparsity must be less than target_sparsity. "
    ...
    def get_sparsity(self, step):
        ...
        current_sparsity = self.initial_sparsity + sparsity_range * (progress ** self.power)
