...
class JAXTrainer(base_trainer.Trainer):
    ...
    def jax_state_sync(self):
        ...
        if trainable_variables:
            for ref_v, v in zip(
                self._compiled_trainable_variables, trainable_variables
            ):
                ref_v.assign(v)
        if non_trainable_variables:
            for ref_v, v in zip(
                self._compiled_non_trainable_variables,
                non_trainable_variables,
            ):
                ref_v.assign(v)
    ...
    def _get_state_sharding_spec(self):
        trainable_shardings = [
            v.value.sharding for v in self._compiled_trainable_variables
        ]
        non_trainable_shardings = [
            v.value.sharding for v in self._compiled_non_trainable_variables
        ]
    ...
    def _check_sharding_consistency(
        ...
        var_shard_pairs = itertools.chain(
            zip(self._compiled_trainable_variables, trainable_shardings),
            zip(
                self._compiled_non_trainable_variables,
                non_trainable_shardings,
            ),
    ...
    def _purge_model_variables(
        ...
        """
        if trainable_variables:
            for v in self._compiled_trainable_variables:
                v._value = None
        if non_trainable_variables:
            for v in self._compiled_non_trainable_variables:
                v._value = None
        if optimizer_variables:
    ...
    def _get_jax_state(
        ...
        state = []
        if trainable_variables:
            state.append([v.value for v in self._compiled_trainable_variables])
        if non_trainable_variables:
            state.append(
                [v.value for v in self._compiled_non_trainable_variables]
            )
        if optimizer_variables:
