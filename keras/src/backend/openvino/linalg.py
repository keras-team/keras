...
def lu_factor(a):
    ...
    range_M = ov_opset.range(zero_const, M_node, one_const, output_type=Type.i32).output(0)
    P_init = ov_opset.broadcast(
        ov_opset.unsqueeze(range_M, zero_const).output(0),
        ov_opset.concat([ov_opset.unsqueeze(B_node, zero_const).output(0), ov_opset.unsqueeze(M_node, zero_const).output(0)], axis=0).output(0)
    ).output(0)
    
    loop = ov_opset.loop(K_node, ov_opset.constant(True, Type.boolean).output(0))
    
    A_body_param = ov_opset.parameter(A_flat.get_partial_shape(), out_ov_type, name="A_curr")
    P_body_param = ov_opset.parameter(P_init.get_partial_shape(), Type.i32, name="P_curr")
    ...
    topk = ov_opset.topk(A_k_col_abs, ov_opset.constant(1, Type.i32), 1, "max", "value")
    p_rel = topk.output(1)
    p_rel_sq = ov_opset.squeeze(p_rel, ov_opset.constant([1, 2], Type.i32).output(0)).output(0)
    p = ov_opset.add(p_rel_sq, k).output(0)
    
    indices = ov_opset.broadcast(
        ov_opset.unsqueeze(ov_opset.range(zero_const, M_body, one_const, output_type=Type.i32).output(0), zero_const).output(0),
        ov_opset.concat([ov_opset.unsqueeze(B_body, zero_const).output(0), ov_opset.unsqueeze(M_body, zero_const).output(0)], axis=0).output(0)
    ).output(0)
    
    B_range = ov_opset.range(zero_const, B_body, one_const, output_type=Type.i32).output(0)
    b_unsqueezed = ov_opset.unsqueeze(B_range, one_const).output(0)
    k_b = ov_opset.broadcast(ov_opset.unsqueeze(k, zero_const).output(0), ov_opset.unsqueeze(B_body, zero_const).output(0)).output(0)
    ...
    A_curr_flat = ov_opset.reshape(A_curr, ov_opset.constant([-1], Type.i32).output(0), False).output(0)
    
    b_coords = ov_opset.broadcast(
        ov_opset.unsqueeze(ov_opset.unsqueeze(B_range, one_const).output(0), ov_opset.constant(2, Type.i32).output(0)).output(0),
    ...
    r_coords = ov_opset.broadcast(
        ov_opset.unsqueeze(indices, ov_opset.constant(2, Type.i32).output(0)).output(0),
        ov_opset.concat([ov_opset.unsqueeze(B_body, zero_const).output(0), ov_opset.unsqueeze(M_body, zero_const).output(0), ov_opset.unsqueeze(N_body, zero_const).output(0)], 0).output(0)
    ).output(0)
    
    c_coords = ov_opset.broadcast(
        ov_opset.unsqueeze(ov_opset.unsqueeze(ov_opset.range(zero_const, N_body, one_const, output_type=Type.i32).output(0), zero_const).output(0), zero_const).output(0),
        ov_opset.concat([ov_opset.unsqueeze(B_body, zero_const).output(0), ov_opset.unsqueeze(M_body, zero_const).output(0), ov_opset.unsqueeze(N_body, zero_const).output(0)], 0).output(0)
    ).output(0)
    ...
    flat_indices_1d = ov_opset.reshape(flat_indices, ov_opset.constant([-1], Type.i32).output(0), False).output(0)
    
    A_swapped_flat = ov_opset.gather(A_curr_flat, flat_indices_1d, zero_const).output(0)
    A_swapped = ov_opset.reshape(A_swapped_flat, ov_opset.concat([ov_opset.unsqueeze(B_body, zero_const).output(0), ov_opset.unsqueeze(M_body, zero_const).output(0), ov_opset.unsqueeze(N_body, zero_const).output(0)], 0).output(0), False).output(0)
    ...
    A_k_k_safe = ov_opset.select(is_zero, ov_opset.constant(1e-10, out_ov_type).output(0), A_k_k).output(0)
    ...
    pads_end = ov_opset.concat([
        ov_opset.constant([0], Type.i32).output(0),
        ov_opset.constant([0], Type.i32).output(0),
        ov_opset.unsqueeze(ov_opset.subtract(N_body, k_plus_1).output(0), zero_const).output(0)
    ], 0).output(0)
    
    L_col_padded = ov_opset.pad(L_col, pads_begin, pads_end, "constant").output(0)
    
    row_range_body = ov_opset.range(zero_const, M_body, one_const, output_type=Type.i32).output(0)
    col_range_body = ov_opset.range(zero_const, N_body, one_const, output_type=Type.i32).output(0)
    ...
    row_mask_3d = ov_opset.broadcast(
        ov_opset.unsqueeze(ov_opset.unsqueeze(row_mask, zero_const).output(0), ov_opset.constant(2, Type.i32).output(0)).output(0),
        ov_opset.shape_of(A_curr, Type.i32).output(0)
    ).output(0)
    col_mask_3d = ov_opset.broadcast(
        ov_opset.unsqueeze(ov_opset.unsqueeze(col_mask, zero_const).output(0), ov_opset.constant(1, Type.i32).output(0)).output(0),
        ov_opset.shape_of(A_curr, Type.i32).output(0)
    ).output(0)
    ...
    P_next = ov_opset.scatter_nd_update(
        P_curr,
        upd_k_indices,
        p
    ).output(0)
    ...
    if rank == 2:
        ...
        A_out_shape = ov_opset.concat([batch_shape, ov_opset.unsqueeze(M_node, zero_const).output(0), ov_opset.unsqueeze(N_node, zero_const).output(0)], 0).output(0)
        P_out_shape = ov_opset.concat([batch_shape, ov_opset.unsqueeze(K_node, zero_const).output(0)], 0).output(0)
