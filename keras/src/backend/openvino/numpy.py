...
def diagonal(x, offset=0, axis1=0, axis2=1):
    ...
    if not (-rank <= axis2 < rank):
        raise ValueError(
            f"axis2 {axis2} is out of bounds for array of dimension {rank}"
        )
    if axis2 < 0:
        axis2 += rank
    ...
    if offset >= 0:
        ...
        max_len = ov_opset.subtract(dim1, get_ov_output(-offset, Type.i64))

    diag_len = ov_opset.maximum(
    ...
    reshape_shape = ov_opset.concat(
        [
            ...
            ov_opset.unsqueeze(dim1, get_ov_output(0, Type.i64)).output(0),
            ov_opset.unsqueeze(dim2, get_ov_output(0, Type.i64)).output(0),
        ],
        axis=0,
    ).output(0)
    ...
    if offset >= 0:
        coords = ov_opset.concat(
            [
                ov_opset.unsqueeze(idx, get_ov_output(1, Type.i64)).output(0),
                ov_opset.unsqueeze(
                    ov_opset.add(idx, get_ov_output(offset, Type.i64)).output(0),
                    get_ov_output(1, Type.i64),
                ).output(0),
            ],
            axis=1,
        ...
        coords = ov_opset.concat(
            [
                ov_opset.unsqueeze(
                    ov_opset.subtract(idx, get_ov_output(offset, Type.i64)).output(0),
                    get_ov_output(1, Type.i64),
                ).output(0),
                ov_opset.unsqueeze(idx, get_ov_output(1, Type.i64)).output(0),
            ],
    ...
    coords = ov_opset.unsqueeze(coords, get_ov_output(0, Type.i64)).output(0)
    coords = ov_opset.broadcast(
        coords,
        ov_opset.concat([batch, diag_len, get_ov_output(2, Type.i64)], axis=0).output(
            0
        ),
        mode="NUMPY",
    ).output(0)
    ...
    if batch_axes:
        ...
        final_shape = ov_opset.concat([batch_shape, diag_len], axis=0).output(0)
    else:
        final_shape = diag_len

    diag = ov_opset.reshape(diag, final_shape, special_zero=False).output(0)
...
def logaddexp2(x1, x2):
    raise NotImplementedError("`logaddexp2` is not supported with openvino backend")
...
def repeat(x, repeats, axis=None):
    ...
    if isinstance(repeats, (int, np.integer)) or (
        ...
        repeats_val = (
            int(repeats) if isinstance(repeats, (np.integer, np.ndarray)) else repeats
        )
        ...
        tiled = ov_opset.tile(idx_range, ov_opset.constant([1, repeats_val], Type.i32))
...
def signbit(x):
    raise NotImplementedError("`signbit` is not supported with openvino backend")
