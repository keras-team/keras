...
def nanmedian(x, axis=None, keepdims=False):
    ...
    if x_type.is_integral() or x_type == Type.boolean:
        return median(OpenVINOKerasTensor(x), axis=axis, keepdims=keepdims)
    ...
    if axis == () or axis == []:
        x1_type = ov_to_keras_type(x_type)
        result_dtype = dtypes.result_type(x1_type, float)
        result_ov_type = OPENVINO_DTYPES[result_dtype]
        if x_type != result_ov_type:
            x = ov_opset.convert(x, result_ov_type).output(0)
        return OpenVINOKerasTensor(x)

    flattened_axes = False
    original_rank = rank
    ...
    if axis is None:
        ...
        if len(axis) == 1:
            ...
            # topk only reduces one axis, so merge target axes into one trailing
            # dim via transpose + reshape before reducing
            axes = sorted(a if a >= 0 else rank + a for a in axis)
            non_axes = [i for i in range(rank) if i not in axes]
            perm = non_axes + axes
            perm_const = ov_opset.constant(perm, Type.i32).output(0)
            x = ov_opset.transpose(x, perm_const).output(0)
            new_rank = len(non_axes) + 1
            shape_tensor = ov_opset.shape_of(x, output_type=Type.i32).output(0)
            non_axes_shape = ov_opset.gather(
                shape_tensor,
                ov_opset.constant(list(range(len(non_axes))), Type.i32).output(
                    0
                ),
                ov_opset.constant(0, Type.i32).output(0),
            ).output(0)
            merged_dim = ov_opset.constant([-1], Type.i32).output(0)
            new_shape = ov_opset.concat([non_axes_shape, merged_dim], 0).output(
                0
            )
            x = ov_opset.reshape(x, new_shape, False).output(0)
            reduce_axis = new_rank - 1
            rank = new_rank
            flattened_axes = True
            original_rank = len(non_axes)
    else:
        reduce_axis = axis
        if reduce_axis < 0:
            reduce_axis = rank + reduce_axis

    # NaNs replaced with +inf so topk(mode="min") sorts them to the end
    nan_mask = ov_opset.is_nan(x)
    pos_inf = ov_opset.constant(np.array(np.inf, dtype=np.float32))
    if x_type != Type.f32:
        pos_inf = ov_opset.convert(pos_inf, x_type)
    ...
    # keepdims=True so the shape stays broadcastable for gather_elements below
    not_nan = ov_opset.logical_not(nan_mask).output(0)
    not_nan_int = ov_opset.convert(not_nan, Type.i32).output(0)
    axis_const_i32 = ov_opset.constant([reduce_axis], Type.i32).output(0)
    valid_count_i32 = ov_opset.reduce_sum(
        not_nan_int, axis_const_i32, True
    ).output(0)

    shape_tensor = ov_opset.shape_of(x_no_nan, output_type=Type.i32).output(0)
    k = ov_opset.gather(
        shape_tensor,
        ov_opset.constant([reduce_axis], Type.i32).output(0),
        ov_opset.constant(0, Type.i32).output(0),
    ).output(0)
    k_scalar = ov_opset.squeeze(k, [0]).output(0)
    topk_outputs = ov_opset.topk(
    ...
    result_dtype = dtypes.result_type(x1_type, float)
    result_ov_type = OPENVINO_DTYPES[result_dtype]
    sorted_values = ov_opset.convert(sorted_values, result_ov_type).output(0)

    # Median indices computed from valid_count, not total slice length
    one_i32 = ov_opset.constant(1, Type.i32).output(0)
    two_i32 = ov_opset.constant(2, Type.i32).output(0)
    vc_minus_1 = ov_opset.subtract(valid_count_i32, one_i32).output(0)
    vc_div_2 = ov_opset.divide(valid_count_i32, two_i32).output(0)
    odd_idx = ov_opset.divide(vc_minus_1, two_i32).output(0)
    even_idx1 = ov_opset.subtract(vc_div_2, one_i32).output(0)
    even_idx2 = vc_div_2
    vc_mod_2 = ov_opset.mod(valid_count_i32, two_i32).output(0)
    is_odd = ov_opset.equal(vc_mod_2, one_i32).output(0)

    odd_result = ov_opset.gather_elements(
        sorted_values, odd_idx, reduce_axis
    ).output(0)
    even_r1 = ov_opset.gather_elements(
        sorted_values, even_idx1, reduce_axis
    ).output(0)
    even_r2 = ov_opset.gather_elements(
        sorted_values, even_idx2, reduce_axis
    ).output(0)

    two_f = ov_opset.constant(2.0, result_ov_type).output(0)
    even_result = ov_opset.divide(
        ov_opset.add(even_r1, even_r2).output(0), two_f
    ).output(0)
    median_result = ov_opset.select(is_odd, odd_result, even_result).output(0)

    zero_i32 = ov_opset.constant(0, Type.i32).output(0)
    all_nan = ov_opset.equal(valid_count_i32, zero_i32).output(0)
    nan_val = ov_opset.constant(np.array(np.nan, dtype=np.float32))
    if result_ov_type != Type.f32:
        nan_val = ov_opset.convert(nan_val, result_ov_type)
    median_result = ov_opset.select(all_nan, nan_val, median_result).output(0)

    # median_result has reduce_axis dim = 1 from keepdims gather_elements
    if not keepdims:
        median_result = ov_opset.squeeze(median_result, [reduce_axis]).output(0)
    elif flattened_axes and axis is None:
