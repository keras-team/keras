...
from keras.src.backend.openvino.ops.core import DTYPES_MAX
from keras.src.backend.openvino.ops.core import DTYPES_MIN
from keras.src.backend.openvino.ops.core import OpenVINOKerasTensor
from keras.src.backend.openvino.ops.core import cast
from keras.src.backend.openvino.ops.core import convert_to_numpy
from keras.src.backend.openvino.ops.core import convert_to_tensor
...
def gaussian_blur(
    ...
    def _create_gaussian_kernel(kernel_size, sigma):
        ...
        def _get_gaussian_kernel2d(size, sigma):
            ...
            kernel1d_height = ov_opset.reshape(
                kernel1d_height,
                ov_opset.constant([int(size[0]), 1], Type.i32).output(0),
                False,
            ).output(0)

            kernel1d_width = ov_opset.reshape(
                kernel1d_width,
                ov_opset.constant([1, int(size[1])], Type.i32).output(0),
                False,
            ).output(0)
            return ov_opset.multiply(kernel1d_height, kernel1d_width).output(0)
