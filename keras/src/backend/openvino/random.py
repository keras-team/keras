...
def _random_uniform(shape, minval, maxval, dtype, seed1, seed2):
    # Pybind11 will sign-flip values >= 2**31.
    # We must mask all seeds to 31-bit integer range to prevent OpenVINO
    # C++ layer from throwing "global_seed should be positive" check failures.
    safe_seed1 = int(seed1) & 0x7FFFFFFF
    safe_seed2 = int(seed2) & 0x7FFFFFFF
