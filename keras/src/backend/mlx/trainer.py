...
class MlxTrainer(base_trainer.Trainer):
    ...
    def train_step(self, data):
        ...
        # Forward pass (pre-update) for the loss tracker and metrics.
        if self._call_has_training_arg:
        ...
        if self.optimizer is not None:
            ...
            if trainable_weights:
                # Functional gradient via MLX autograd. We re-run the forward
                # inside `loss_fn` with the trainable variables bound to fresh
                # leaves through a StatelessScope, so `mx.value_and_grad`
                # differentiates the loss w.r.t. those leaf values. MLX arrays
                # are immutable in the autograd sense, so the optimizer's
                # `apply` rebinds each Variable to a new updated value.
                values = [w.value for w in trainable_weights]

                def loss_fn(*vals):
                    with StatelessScope(
                        state_mapping=list(zip(trainable_weights, vals)),
                        collect_losses=True,
                    ):
                        if self._call_has_training_arg:
                        ...
                        # Scale the loss for mixed-precision (float16) gradient
                        # computation to avoid underflow; for a plain optimizer
                        # `scale_loss` is a no-op (scale=1), and
                        # `LossScaleOptimizer.apply` unscales grads before the
                        # update.
                        return self.optimizer.scale_loss(loss)

                _, grads = mx.value_and_grad(
    ...
    def make_train_function(self, force=False):
        ...
        if self.steps_per_execution > 1:
            raise ValueError(
                "`steps_per_execution` must be 1 with the MLX backend. "
    ...
    def predict(
        ...
        return tree.map_structure_up_to(
            batch_outputs,
            lambda xs: convert_to_numpy(mx.concatenate(xs)),
            outputs,
        )
