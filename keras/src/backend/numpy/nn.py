...
def fold(input, output_size, kernel_size, dilation=1, padding=0, stride=1):
    ...
    output = np.zeros((N, C, H_padded, W_padded), dtype=input.dtype)

    # Build indices for scatter
    for i in range(kH):
        for j in range(kW):
            # Compute the starting positions for this kernel element
            h_positions = np.arange(nH) * sH + i * dH
            w_positions = np.arange(nW) * sW + j * dW
            ...
            np.add.at(output, (n_idx, c_idx, h_idx, w_idx), vals)
