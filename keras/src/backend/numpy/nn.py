...
def _adaptive_pool1d_impl(inputs, output_size, mode, data_format):
    ...
    small_pool = (
        np.mean(sv_small, axis=2) if mode == "avg" else np.max(sv_small, axis=2)
    )

    sv_big = _strided_view_1d(inputs, big)
    big_pool = (
        np.mean(sv_big, axis=2) if mode == "avg" else np.max(sv_big, axis=2)
    )
...
def _adaptive_pool2d_impl(inputs, output_size, mode, data_format):
    ...
    big_pool_h = (
        np.mean(sv_big_h, axis=2) if mode == "avg" else np.max(sv_big_h, axis=2)
    )
    ...
    big_pool_w = (
        np.mean(sv_big_w, axis=2) if mode == "avg" else np.max(sv_big_w, axis=2)
    )
...
def _adaptive_pool3d_impl(inputs, output_size, mode, data_format):
    ...
    big_pool_d = (
        np.mean(sv_big_d, axis=2) if mode == "avg" else np.max(sv_big_d, axis=2)
    )
...
def adaptive_avg_pool(inputs, output_size, data_format="channels_first"):
    dims = inputs.ndim - 2
    if dims == 1:
        return _adaptive_pool1d_impl(inputs, output_size, "avg", data_format)
    if dims == 2:
        return _adaptive_pool2d_impl(inputs, output_size, "avg", data_format)
    if dims == 3:
        return _adaptive_pool3d_impl(inputs, output_size, "avg", data_format)
    raise ValueError("adaptive_avg_pool supports only 1D/2D/3D")
