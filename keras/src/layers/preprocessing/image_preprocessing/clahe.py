...
class ContrastLimitedAdaptiveHistogramEqualization(BaseImagePreprocessingLayer):
    ...
    **Note:** This layer computes histograms using
    `self.backend.ops.nn.one_hot`, which can be highly memory-intensive. For
    large batch sizes or high-resolution images, it may lead to high memory
    consumption or out-of-memory errors.
    ...
    def transform_images(self, images, transformation=None, training=True):
        if self.data_format == "channels_first":
            if len(images.shape) == 4:
                images = self.backend.ops.numpy.transpose(images, (0, 2, 3, 1))
            else:
                images = self.backend.ops.numpy.transpose(images, (1, 2, 0))
        ...
        images = self.backend.ops.cast(images, original_dtype)

        if self.data_format == "channels_first":
            if len(images.shape) == 4:
                images = self.backend.ops.numpy.transpose(images, (0, 3, 1, 2))
            else:
                images = self.backend.ops.numpy.transpose(images, (2, 0, 1))
    ...
    def _clahe(self, images):
        ...
        if (
            ...
            images_nchw = self.backend.ops.numpy.transpose(images, (0, 3, 1, 2))

            images_3d = self.backend.ops.numpy.reshape(
                images_nchw, (-1, height, width)
            )
            padded_3d = self.backend.ops.numpy.pad(
                images_3d, [[0, 0], [0, pad_h], [0, pad_w]], mode="symmetric"
            )
            padded_nchw = self.backend.ops.numpy.reshape(
                padded_3d, (-1, channels, height + pad_h, width + pad_w)
            )
            padded_images = self.backend.ops.numpy.transpose(
                padded_nchw, (0, 2, 3, 1)
            )

        # Compute Histograms per tile
        tiled = self.backend.ops.numpy.reshape(
            padded_images,
            (batch_size, grid_h, tile_h, grid_w, tile_w, channels),
        )
        tiled = self.backend.ops.numpy.transpose(tiled, (0, 1, 3, 5, 2, 4))

        tiled_flat = self.backend.ops.numpy.reshape(
            tiled, (batch_size, grid_h, grid_w, channels, tile_h * tile_w)
        )

        tiled_int = self.backend.ops.cast(tiled_flat, "int32")
        tiled_int = self.backend.ops.numpy.clip(tiled_int, 0, 255)

        # Compute histograms via one_hot and sum
        hists = self.backend.ops.numpy.sum(
            self.backend.ops.nn.one_hot(tiled_int, 256), axis=-2
        )

        # Clip and redistribute
        if self.clip_limit > 0:
            limit = self.clip_limit * (tile_h * tile_w) / 256.0
            limit = self.backend.ops.cast(limit, hists.dtype)

            clipped = self.backend.ops.numpy.clip(hists, 0, limit)

            excess = self.backend.ops.numpy.sum(
        ...
        # Compute CDF
        cdf = self.backend.ops.numpy.cumsum(hists, axis=-1)
        cdf_min = self.backend.ops.numpy.min(cdf, axis=-1, keepdims=True)

        numerator = (cdf - cdf_min) * 255.0
        denominator = (
            self.backend.ops.cast(tile_h * tile_w, cdf.dtype) - cdf_min
        )

        denominator = self.backend.ops.numpy.where(
            denominator == 0,
            self.backend.ops.numpy.ones_like(denominator),
            denominator,
        )
        cdf_norm = numerator / denominator
        cdf_norm = self.backend.ops.numpy.clip(cdf_norm, 0, 255)

        # Interpolation

        top = cdf_norm[:, 0:1, :, :, :]
        bottom = cdf_norm[:, -1:, :, :, :]
        cdf_padded = self.backend.ops.numpy.concatenate(
        ...
        y_range = self.backend.ops.numpy.arange(H_padded, dtype="float32")
        x_range = self.backend.ops.numpy.arange(W_padded, dtype="float32")
        ...
        y0 = self.backend.ops.numpy.floor(y_grid)
        x0 = self.backend.ops.numpy.floor(x_grid)
        y1 = y0 + 1.0
        x1 = x0 + 1.0
        ...
        y0 = self.backend.ops.numpy.clip(y0, 0, grid_h + 1)
        y1 = self.backend.ops.numpy.clip(y1, 0, grid_h + 1)
        x0 = self.backend.ops.numpy.clip(x0, 0, grid_w + 1)
        x1 = self.backend.ops.numpy.clip(x1, 0, grid_w + 1)

        y0 = self.backend.ops.cast(y0, "int32")
        y1 = self.backend.ops.cast(y1, "int32")
        x0 = self.backend.ops.cast(x0, "int32")
        x1 = self.backend.ops.cast(x1, "int32")
        ...
        cdf_flat = self.backend.ops.numpy.reshape(cdf_padded, (-1,))

        pixels = self.backend.ops.cast(
            self.backend.ops.numpy.clip(padded_images, 0, 255), "int32"
        )

        b_idx = self.backend.ops.numpy.arange(batch_size, dtype="int32")[
            :, None, None, None
        ]

        c_idx = self.backend.ops.numpy.arange(channels, dtype="int32")[
            None, None, None, :
        ]
        ...
        val_nw = self.backend.ops.numpy.take(cdf_flat, idx_nw)

        idx_ne = base_idx + y0_e * stride_y + x1_e * stride_x
        val_ne = self.backend.ops.numpy.take(cdf_flat, idx_ne)

        idx_sw = base_idx + y1_e * stride_y + x0_e * stride_x
        val_sw = self.backend.ops.numpy.take(cdf_flat, idx_sw)

        idx_se = base_idx + y1_e * stride_y + x1_e * stride_x
        val_se = self.backend.ops.numpy.take(cdf_flat, idx_se)
        ...
        if unbatched:
            result = self.backend.ops.numpy.squeeze(result, axis=0)
