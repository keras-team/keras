...
def assert_input_compatibility(input_spec, inputs, layer_name):
    ...
    # Fast path: single InputSpec and single tensor input (the common case).
    # Avoids two tree.flatten calls.
    if isinstance(input_spec, InputSpec) and not isinstance(inputs, dict):
        if hasattr(inputs, "shape"):
            _check_single_input(inputs, input_spec, 0, layer_name)
            return
        # inputs might be a list/tuple with one element
        flat_inputs = tree.flatten(inputs)
        if len(flat_inputs) != 1:
            raise ValueError(
                f'Layer "{layer_name}" expects 1 input(s),'
                f" but it received {len(flat_inputs)} input tensors. "
                f"Inputs received: {flat_inputs}"
            )
        _check_single_input(flat_inputs[0], input_spec, 0, layer_name)
        return
    ...
    for input_index, (x, spec) in enumerate(zip(inputs, input_spec)):
        _check_single_input(x, spec, input_index, layer_name)


def _check_single_input(x, spec, input_index, layer_name):
    if spec is None:
        return
