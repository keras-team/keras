...
from keras.src.pruning.core import get_model_sparsity, apply_pruning_to_layer
...
class PruningCallback(Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        if self.config.should_prune_at_step(self.current_step):
            current_sparsity = self.config.get_sparsity_for_step(self.current_step)
            
            # Apply pruning to all eligible layers
            pruned_layers = 0
            for layer in self.model.layers:
                if apply_pruning_to_layer(layer, current_sparsity, self.config.method):
                    pruned_layers += 1
...
class PostTrainingPruning(Callback):
    ...
    def on_train_end(self, logs=None):
        ...
        pruned_layers = 0
        for layer in self.model.layers:
            if apply_pruning_to_layer(layer, self.config.sparsity, self.config.method):
                pruned_layers += 1
