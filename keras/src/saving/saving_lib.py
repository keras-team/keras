...
def _save_state(
    ...
    if not isinstance(
        weights_store,
        (H5IOStore, ShardedH5IOStore, NpzIOStore, type(None)),
    ):
        raise ValueError(
    ...
    if not isinstance(assets_store, (DiskIOStore, DirectIOStore, type(None))):
        raise ValueError(
...
def _save_container_state(
    ...
    for saveable in container:
        if isinstance(saveable, KerasSaveable):
            # For Layers, use the layer's name attribute as it's stable
            # and part of the Keras API contract. For other saveables,
            # use class name.
            if (
                isinstance(saveable, Layer)
