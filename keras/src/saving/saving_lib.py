...
def _save_state(
    ...
    if weights_store is not None and not isinstance(
        weights_store,
        (H5IOStore, ShardedH5IOStore, NpzIOStore),
    ):
        raise ValueError(
...
def _save_container_state(
    ...
    # When saving assets only (no weights), use layer names for stable paths
    # When saving weights, use class names for H5 backward compatibility
    use_layer_names = weights_store is None and assets_store is not None
    ...
    for saveable in container:
        if isinstance(saveable, KerasSaveable):
            if (
                use_layer_names
                and isinstance(saveable, Layer)
                and hasattr(saveable, "name")
                and saveable.name
            ):
                # Use layer.name for assets (stable, part of Keras API)
                name = saveable.name
            else:
                # Use class name for weights (backward compatible with H5)
                # Do NOT use saveable.name for weights, since names are
                # usually autogenerated and thus not reproducible
...
def _load_container_state(
    ...
    # When loading assets only (no weights), use layer names for stable paths
    # When loading weights, use class names for H5 backward compatibility
    use_layer_names = weights_store is None and assets_store is not None
