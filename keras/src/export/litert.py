...
class LiteRTExporter:
    ...
    def export(self, filepath):
        ...
        if not tree.is_nested(self.input_signature):
            # Case 1: Single input - use as-is
            model_to_convert = self.model
            signature_for_conversion = self.input_signature
        elif isinstance(self.input_signature, list) and len(
            self.input_signature
        ) == len(tree.flatten(self.input_signature)):
            ...
            adapted_model = self._create_nested_inputs_adapter(
                self.input_signature
            )

            # Flatten signature for TFLite conversion
            signature_for_conversion = tree.flatten(self.input_signature)
    ...
    def _create_nested_inputs_adapter(self, input_signature_struct):
        ...
        for path, spec in paths_and_specs:
            # Extract the input name from the path
            if path:
                # path is a tuple of keys, use the last one as the name
                name = str(path[-1])
            else:
                name = "input"
