...
class HasList(training.Model):
  ...
  def __init__(self):
    ...
    self.layer_list.extend(
        [core.Dense(5),
         core.Dense(6, kernel_regularizer=tf.compat.v2.math.reduce_sum)])
    self.layer_list += [
        core.Dense(7, bias_regularizer=tf.compat.v2.math.reduce_sum),
        core.Dense(8)
    ]
    self.layer_list += (
  ...
  def call(self, x):
    ...
    for l in self.layer_list:
      x = l(x)
      aggregation += tf.compat.v2.math.reduce_sum(x)
...
class HasTuple(training.Model):
  ...
  def __init__(self):
    ...
    self.layer_list = (
        ...
        core.Dense(5, kernel_regularizer=tf.compat.v2.math.reduce_sum))
