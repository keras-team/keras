  ...
  def train_step(x, y, w=None):
    with tf.GradientTape() as tape:
      if w is not None:
        ...
        model([x, y])
      loss = tf.compat.v2.math.reduce_sum(model.losses)
...
class TestAddLossCorrectness(keras_parameterized.TestCase):
  ...
  def test_loss_on_model_fit(self):
    ...
    model.add_loss(tf.compat.v2.math.reduce_mean(mae(targets, outputs)))
    model.compile(
  ...
  def test_loss_callable_on_model_fit(self):
    ...
    def callable_loss():
      return tf.compat.v2.math.reduce_sum(model.weights)
  ...
  def test_loss_with_sample_weight_on_model_fit(self):
    ...
    model.add_loss(3 * tf.compat.v2.math.reduce_mean(sw * mae(targets, outputs)))
    model.compile(
  ...
  def test_loss_with_sample_weight_on_model_ctl(self):
    def get_model_and_train_step():
      ...
      model = Model([inputs, targets, sw], outputs)
      model.add_loss(MAE()(targets, outputs, sw))
      model.add_loss(tf.compat.v2.math.reduce_mean(sw * mae(targets, outputs)))
      return get_ctl_train_step(model)
  ...
  def test_loss_with_sample_weight_in_model_call(self):
    ...
    class MyModel(Model):
      ...
      def call(self, inputs):
        ...
        self.add_loss(tf.compat.v2.math.reduce_mean(inputs[2] * mae(inputs[1], outputs)))
        return outputs
  ...
  def test_loss_with_sample_weight_in_layer_call(self):
    ...
    class MyLayer(layers.Layer):
      ...
      def call(self, inputs):
        ...
        self.add_loss(tf.compat.v2.math.reduce_mean(inputs[2] * mae(inputs[1], out)))
        return out
