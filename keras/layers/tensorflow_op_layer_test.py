...
def _single_op_with_attrs():
  inputs = keras.Input(shape=(10,))
  x = tf.compat.v2.math.reduce_mean(inputs, axis=1, keepdims=True)
  outputs = keras.layers.Dense(10)(x)
  return keras.Model(inputs, outputs)
...
def _reuse_ancillary_layer():
  ...
  l2_loss = tf.compat.v2.math.reduce_mean(
      tf.compat.v2.math.reduce_sum(tf.math.square(outputs_delta), -1))
  model.add_loss(l2_loss)
  model.add_metric(l2_loss, aggregation='mean', name='l2_loss')
  l1_loss = 0.01 * tf.compat.v2.math.reduce_mean(
      tf.compat.v2.math.reduce_sum(tf.abs(outputs_delta), -1))
...
class AutoLambdaTest(keras_parameterized.TestCase):
  ...
  def test_numerical_correctness_with_attrs(self):
    ...
    outputs = tf.compat.v2.math.reduce_mean(inputs, axis=1)
    model = keras.Model(inputs, outputs)
    y = self.evaluate(model(x))
  ...
  def test_gradient_tape_in_function(self):
    ...
    x = tf.compat.v2.math.reduce_mean(x, axis=0, keepdims=True)
    h = tf.nn.relu(x)
    m = keras.Model(z, h)
