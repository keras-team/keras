...
class GRU(recurrent.DropoutRNNCellMixin, recurrent.GRU):
  ...
  def __init__(self,
    ...
    self._could_use_gpu_kernel = (
        self.activation in (activations.tanh, tf.math.tanh) and
        self.recurrent_activation in (activations.sigmoid, tf.math.sigmoid) and
...
def standard_gru(inputs, init_h, kernel, recurrent_kernel, bias, mask,
  ...
  def step(cell_inputs, cell_states):
    ...
    hh = tf.math.tanh(x_h + r * recurrent_h)
...
def is_sequence_right_padded(mask):
  ...
  count_of_true = tf.compat.v2.math.reduce_sum(tf.cast(mask, tf.dtypes.int32), axis=1)
  right_padded_mask = tf.sequence_mask(
      count_of_true, maxlen=max_seq_length)
  return tf.compat.v2.math.reduce_all(tf.equal(mask, right_padded_mask))
...
def has_fully_masked_sequence(mask):
  ...
  return tf.compat.v2.math.reduce_any(
...
def calculate_sequence_by_mask(mask, time_major):
  ...
  return tf.compat.v2.math.reduce_sum(tf.cast(mask, tf.dtypes.int32),
                             axis=timestep_index)
