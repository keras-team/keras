...
class ConvLSTM2D_(ConvRecurrent2D):
    """Convolutional LSTM.
    ...
    def __init__(self, filters,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(ConvLSTM2D_, self).__init__(filters,
                                         kernel_size,
                                         strides=strides,
                                         padding=padding,
    ...
    def get_config(self):
        ...
        base_config = super(ConvLSTM2D_, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
class RNN2D(Layer):
    ...
    # Arguments
        cell: A RNN2D cell instance. A RNN2D cell is a class that has:
            ...
            It is also possible for `cell` to be a list of RNN2D cell instances,
            in which cases the cells get stacked on after the other in the RNN2D,
            implementing an efficient stacked RNN2D.
        return_sequences: Boolean. Whether to return the last output.
            in the output sequence, or the full sequence.
        ...
        unroll: Boolean (default False).
            ...
            Unrolling can speed-up a RNN2D,
    ...
    # Note on using statefulness in RNN2Ds
        You can set RNN2D layers to be 'stateful', which means that the states
        computed for the samples in one batch will be reused as initial states
    ...
    # Note on specifying the initial state of RNN2Ds
        You can specify the initial state of RNN2D layers symbolically by
        calling them with the keyword argument `initial_state`. The value of
        `initial_state` should be a tensor or list of tensors representing
        the initial state of the RNN2D layer.

        You can specify the initial state of RNN2D layers numerically by
        calling `reset_states` with the keyword argument `states`. The value of
        `states` should be a numpy array or list of numpy arrays representing
        the initial state of the RNN2D layer.

    # Note on passing external constants to RNN2Ds
        You can pass "external" constants to the cell using the `constants`
        keyword argument of `RNN2D.__call__` (as well as `RNN2D.call`) method. This
    ...
    ```python
        # First, let's define a RNN2D Cell, as a layer subclass.

        class MinimalRNN2DCell(keras.layers.Layer):

            def __init__(self, units, **kwargs):
                self.units = units
                self.state_size = units
                super(MinimalRNN2DCell, self).__init__(**kwargs)
        ...
        # Let's use this cell in a RNN2D layer:

        cell = MinimalRNN2DCell(32)
        x = keras.Input((None, 5))
        layer = RNN2D(cell)
        y = layer(x)

        # Here's how to use the cell to build a stacked RNN2D:

        cells = [MinimalRNN2DCell(32), MinimalRNN2DCell(64)]
        x = keras.Input((None, 5))
        layer = RNN(cells)
        y = layer(x)
    ```
