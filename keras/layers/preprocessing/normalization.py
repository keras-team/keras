...
class Normalization(base_preprocessing_layer.PreprocessingLayer):
  ...
  def update_state(self, data):
    ...
    batch_count = tf.compat.v2.math.reduce_prod(batch_reduce_shape)
  ...
  def merge_state(self, layers):
    ...
    total_count = tf.compat.v2.math.reduce_sum(layer_counts)
    layer_weightings = (
    ...
    total_mean = tf.compat.v2.math.reduce_sum(layer_means * layer_weightings, axis=0)
    inter_layer_variances = (layer_means - total_mean)**2
    total_variance = tf.compat.v2.math.reduce_sum(
