...
def get_reduce_op(reduction_str):
  ...
  if reduction_str == "max":
    return tf.compat.v2.math.reduce_max
  elif reduction_str == "mean":
    return tf.compat.v2.math.reduce_mean
  elif reduction_str == "min":
    return tf.compat.v2.math.reduce_min
  elif reduction_str == "prod":
    return tf.compat.v2.math.reduce_prod
...
class Reduction(Layer):
  ...
  def call(self, inputs, weights=None):
    ...
    weighted_inputs = tf.math.multiply(inputs, weights)
    ...
    if self.reduction == "mean":
      input_sum = tf.compat.v2.math.reduce_sum(weighted_inputs, axis=self.axis)
      weight_sum = tf.compat.v2.math.reduce_sum(weights, axis=self.axis)
    ...
    if self.reduction == "sqrtn":
      ...
      input_sum = tf.compat.v2.math.reduce_sum(weighted_inputs, axis=self.axis)
      squared_weights = tf.math.pow(weights, 2)
      squared_weights_sum = tf.compat.v2.math.reduce_sum(squared_weights, axis=self.axis)
      sqrt_weights_sum = tf.math.sqrt(squared_weights_sum)
