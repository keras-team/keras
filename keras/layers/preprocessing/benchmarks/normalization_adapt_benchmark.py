...
def reduce_fn(state, values):
  ...
  sum_v = tf.compat.v2.math.reduce_sum(values, axis=0)
  sum_v2 = tf.compat.v2.math.reduce_sum(tf.math.square(values), axis=0)
  ones = tf.compat.v1.ones_like(values, dtype=tf.dtypes.int32)
  batch_size = tf.compat.v2.math.reduce_sum(ones, axis=0)
  batch_size_f = tf.cast(batch_size, tf.dtypes.float32)

  ex = 0 + sum_v - tf.math.multiply(batch_size_f, k)
  ex2 = 0 + sum_v2 + tf.math.multiply(
      batch_size_f, (tf.math.square(k) -
