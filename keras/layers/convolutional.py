...
class Deconvolution2D(Layer):
    '''Deconvolution operator for filtering windows of two-dimensional inputs.
    This operation is also known as transposed/inverse convolution. There is
    nothing fundamentaly different from a regular convolution, but the fact that
    we swap the forward and backward/gradient operators. There, deconvolution
    `strides` upsample the resulting image.
    ...
    # Arguments
        ...
        weights: list of numpy arrays to set as initial weights.
        border_mode: 'valid' or 'same'.
        upsample: tuple of length 2. Factor by which to upsample output.
            Also called strides elsewhere.
        W_regularizer: instance of [WeightRegularizer](../regularizers.md)
            (eg. L1 or L2 regularization), applied to the main weights matrix.
        b_regularizer: instance of [WeightRegularizer](../regularizers.md),
    ...
    def __init__(self, nb_filter, nb_row, nb_col,
                 init='glorot_uniform', activation='linear', weights=None,
                 border_mode='valid', upsample=(1, 1), dim_ordering=K.image_dim_ordering(),
                 W_regularizer=None, b_regularizer=None, activity_regularizer=None,
                 W_constraint=None, b_constraint=None,
                 bias=True, **kwargs):
    ...
    def get_output_shape_for(self, input_shape):
        ...
        rows = deconv_output_length(rows, self.nb_row,
                                    self.border_mode, self.subsample[0])
        cols = deconv_output_length(cols, self.nb_col,
                                    self.border_mode, self.subsample[1])
    ...
    def call(self, x, mask=None):
        ...
        output = K.deconv2d(x, self.W, strides=self.upsample,
                            border_mode=self.border_mode,
                            dim_ordering=self.dim_ordering,
                            filter_shape=self.W_shape,
