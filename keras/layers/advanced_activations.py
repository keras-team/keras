...
class MPELU(Layer):
    ...
    # References
        - [Improving Deep Neural Network with Multiple Parametric Exponential Linear Units](http://arxiv.org/abs/1606.00305)
    '''
    def __init__(self, alpha_init=0.0, beta_init=1.0, weights=None, **kwargs):
        self.supports_masking = True
        self.alpha_init = K.cast_to_floatx(alpha_init)
        self.beta_init = K.cast_to_floatx(beta_init)
        self.initial_weights = weights
    ...
    def call(self, x, mask=None):
        pos = K.relu(x)
        neg = self.alphas * (K.exp((x - abs(x)) * 0.5 * self.betas) - 1)
        return pos + neg

    def get_config(self):
        config = {'alpha_init': self.alpha_init,
                  'beta_init': self.beta_init}
        base_config = super(MPELU, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
