...
class TLU(Layer):
    """Trainable Rectified Linear Unit.

    It follows:
    `f(x) = alpha * x for x < 0`,
    `f(x) =  beta * x for x >= 0`,
    where `alpha` and `beta` are learned array with the same shape as x.
    ...
    # References
        - [EM-net: Deep learning for electron microscopy 
        image segmentation](https://www.biorxiv.org/content/10.1101/2020.02.03.933127v1)
    """
