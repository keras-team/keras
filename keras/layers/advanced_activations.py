...
class PReLU(Layer):
    ...
    # Arguments
        ...
        param_share_axes: the axes along with to share parameters for
                          ...
                          param_share_axes = [1, 2]

    # References
        - [Delving Deep into Rectifiers: Surpassing Human-Level Performance on ImageNet Classification](http://arxiv.org/pdf/1502.01852v1.pdf)
    '''
    def __init__(self, init='zero', weights=None, param_share_axes=None, **kwargs):
        self.supports_masking = True
        self.init = initializations.get(init)
        self.initial_weights = weights
        if type(param_share_axes) is not list and type(param_share_axes) is not tuple:
            self.param_share_axes = [param_share_axes]
        else:
            self.param_share_axes = list(param_share_axes)
        super(PReLU, self).__init__(**kwargs)

    def build(self, input_shape):
        param_shape = list(input_shape[1:])
        self.param_broadcast = [False] * len(param_shape)
        if self.param_share_axes[0] is not None:
            for i in self.param_share_axes:
                param_shape[i] = 1
...
class ParametricSoftplus(Layer):
    ...
    def __init__(self, alpha_init=0.2, beta_init=5.0,
                 weights=None, param_share_axes=None, **kwargs):
        self.supports_masking = True
...
class SReLU(Layer):
    ...
    def __init__(self, t_left_init='zero', a_left_init='glorot_uniform',
                 t_right_init='glorot_uniform', a_right_init='one', param_share_axes=None, **kwargs):
        self.supports_masking = True
