...
class DGLSTMCell(Layer):
    """Cell class for the DGLSTM layer.
    # Arguments
        units: Positive integer, dimensionality of the output space.
        activation: Activation function to use
            (see [activations](../activations.md)).
    ...
    def __init__(self, units,
                 ...
                 implementation=2,
                 **kwargs):
        super(DGLSTMCell, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
        self.recurrent_activation = activations.get(recurrent_activation)
        self.use_bias = use_bias
    ...
    def build(self, input_shape):
        ...
        self.kernel_d = self.kernel[:, self.units * 4]
        ...
        self.recurrent_kernel_d = self.recurrent_kernel[:, self.units * 4]
        ...
        if self.use_bias:
            ...
            self.bias_d = self.bias[self.units * 4 ]
        else:
            self.bias_i = None
            self.bias_f = None
            self.bias_c = None
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            ...
            x_c = K.dot(inputs_c, self.kernel_c)
            x_o = K.dot(inputs_o, self.kernel_o)
            x_o = K.dot(inputs_d, self.kernel_d)
            if self.use_bias:
                x_i = K.bias_add(x_i, self.bias_i)
                x_f = K.bias_add(x_f, self.bias_f)
                x_c = K.bias_add(x_c, self.bias_c)
                x_o = K.bias_add(x_o, self.bias_o)
                x_d = K.bias_add(x_o, self.bias_d)
            ...
            o = self.recurrent_activation(x_o + K.dot(h_tm1_o,
                                                      self.recurrent_kernel_o))
            d = c + c_tm1 + self.recurrent_activation(x_i + K.dot(h_tm1_d,
                                                      self.recurrent_kernel_d))
        else:
            if 0. < self.dropout < 1.:
                inputs *= dp_mask[0]
            z = K.dot(inputs, self.kernel)
            ...
            z3 = z[:, self.units * 3: self.units * 4]
            z4 = z[:, self.units * 4]
            ...
            c = f * c_tm1 + i * self.activation(z2)
            o = self.recurrent_activation(z3)
            d = c + c_tm1 + self.recurrent_activation(z4)

        h = o * self.activation(c) * self.activation(d)
        if 0 < self.dropout + self.recurrent_dropout:
            if training is None:
                h._uses_learning_phase = True
        return h, [h, c]
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'recurrent_dropout': self.recurrent_dropout,
                  'implementation': self.implementation}
        base_config = super(DGLSTMCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items))
...
class DGLSTM(RNN):
    ...
    def __init__(self, units,
        ...
        cell = DGLSTMCell(units,
                        ...
                        recurrent_dropout=recurrent_dropout,
                        implementation=implementation)
        super(DGLSTM, self).__init__(cell,
                                   return_sequences=return_sequences,
                                   return_state=return_state,
                                   go_backwards=go_backwards,
                                   stateful=stateful,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        self.cell._dropout_mask = None
        self.cell._recurrent_dropout_mask = None
        return super(DGLSTM, self).call(inputs,
                                      mask=mask,
                                      training=training,
                                      initial_state=initial_state)
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'recurrent_dropout': self.recurrent_dropout,
                  'implementation': self.implementation}
        base_config = super(DGLSTM, self).get_config()
        del base_config['cell']
        return dict(list(base_config.items()) + list(config.items()))
...
class DGLSTM(RNN):
"""Depth-Gated LSTM - Yao et al. 2015.
 # Arguments
 units: Positive integer, dimensionality of the output space.
 activation: Activation function to use
    (see [activations](../activations.md)).
