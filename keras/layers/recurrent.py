...
class LSTM_Cond(Recurrent):
    ...
    def __init__(self, output_dim, embedding_size,
        ...
        if self.dropout_W or self.dropout_U or self.dropout_V:
            self.uses_learning_phase = True
        super(LSTM_Cond, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        if self.consume_less == 'gpu':
            ...
            self.b = K.variable(np.hstack((np.zeros(self.output_dim),
                                           ...
                                           np.zeros(self.output_dim))),
                                name='{}_b'.format(self.name))
            self.trainable_weights = [self.W, self.U, self.V, self.b]
    ...
    def preprocess_input(self, x):
        if self.consume_less == 'cpu':
            ...
            if 0 < self.dropout_W < 1:
                ...
                dropout_W = 0
            input_shape = self.input_spec[0].shape
            input_dim = input_shape[2] - self.embedding_size
            timesteps = input_shape[1]

            x_i = time_distributed_dense(x, self.W_i, self.b_i, dropout_W,
                                         input_dim, self.output_dim, timesteps)
            x_f = time_distributed_dense(x, self.W_f, self.b_f, dropout_W,
                                         input_dim, self.output_dim, timesteps)
            x_c = time_distributed_dense(x, self.W_c, self.b_c, dropout_W,
                                         input_dim, self.output_dim, timesteps)
            x_o = time_distributed_dense(x, self.W_o, self.b_o, dropout_W,
                                         input_dim, self.output_dim, timesteps)
            if 0 < self.dropout_V < 1:
            ...
            input_dim = self.embedding_size
            timesteps = input_shape[1]
            s_i = time_distributed_dense(state_below, self.V_i, self.b_i, dropout_V,
                                        input_dim, self.output_dim, timesteps)
            s_f = time_distributed_dense(state_below, self.V_f, self.b_f, dropout_V,
                                        input_dim, self.output_dim, timesteps)
            s_c = time_distributed_dense(state_below, self.V_c, self.b_c, dropout_V,
                                        input_dim, self.output_dim, timesteps)
            s_o = time_distributed_dense(state_below, self.V_o, self.b_o, dropout_V,
                                        input_dim, self.output_dim, timesteps)
            return [K.concatenate([x_i, x_f, x_c, x_o], axis=2),
                    K.concatenate([s_i, s_f, s_c, s_o], axis=2)]
        else:
            return x
    ...
    def get_config(self):
        config = {"output_dim": self.output_dim,
                  ...
                  "dropout_U": self.dropout_U,
                  "dropout_V": self.dropout_V}
        base_config = super(LSTM_Cond, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
