...
def extract_volume_patches(
    ...
    """TensorFlow implementation of extract_volume_patches.

    TensorFlow has native support via tf.extract_volume_patches.
    """
    import tensorflow as tf
    ...
    if isinstance(size, int):
        ksizes = [1, size, size, size, 1]
    else:
        ksizes = [1] + list(size) + [1]

    if isinstance(strides, int):
        strides_tf = [1, strides, strides, strides, 1]
    else:
        strides_tf = [1] + list(strides) + [1]

    if isinstance(dilation_rate, (list, tuple)):
        if any(d != 1 for d in dilation_rate):
            raise NotImplementedError(
                "dilation_rate > 1 is not supported for TensorFlow backend "
                "with extract_volume_patches."
            )
    ...
    patches = tf.extract_volume_patches(
        volumes, ksizes=ksizes, strides=strides_tf, padding=padding.upper()
    )
    return patches
