...
def extract_volume_patches(
    ...
    """PyTorch implementation of extract_volume_patches."""
    if data_format == "channels_last":
        # (B, D, H, W, C) -> (B, C, D, H, W)
    ...
    if padding == "same":
        ...
        pad_d = max((depth - 1 + sd) // sd * sd + eff_pd - depth - sd, 0)
        pad_h = max((height - 1 + sh) // sh * sh + eff_ph - height - sh, 0)
        pad_w = max((width - 1 + sw) // sw * sw + eff_pw - width - sw, 0)

        # PyTorch pad expects (left, right, top, bottom, front, back)
        volumes = F.pad(
            volumes,
            (
                pad_w // 2,
        ...
        depth, height, width = volumes.shape[2:5]

    if dd == 1 and dh == 1 and dw == 1:
        # No dilation - can use unfold directly
        patches = volumes.unfold(2, pd, sd).unfold(3, ph, sh).unfold(4, pw, sw)
        # patches shape: (B, C, out_d, out_h, out_w, pd, ph, pw)

        # Rearrange to (B, out_d, out_h, out_w, C*pd*ph*pw)
        batch, c, out_d, out_h, out_w, _, _, _ = patches.shape
        patches = patches.permute(0, 2, 3, 4, 1, 5, 6, 7).contiguous()
        patches = patches.reshape(batch, out_d, out_h, out_w, -1)
    else:
        # With dilation - manual extraction
        eff_pd = pd + (pd - 1) * (dd - 1)
        eff_ph = ph + (ph - 1) * (dh - 1)
        eff_pw = pw + (pw - 1) * (dw - 1)
        ...
        patches = all_patches.reshape(batch_size, out_d, out_h, out_w, -1)
