...
def extract_volume_patches(
    ...
    """NumPy implementation of extract_volume_patches."""
    import numpy as np

    # Convert to channels_last if needed
    if data_format == "channels_first":
        # (B, C, D, H, W) -> (B, D, H, W, C)
        volumes = np.transpose(volumes, [0, 2, 3, 4, 1])
    ...
    # Extract patches
    patch_size = pd * ph * pw * channels
    patches = np.zeros(
        (batch_size, out_d, out_h, out_w, patch_size), dtype=volumes.dtype
    )
    ...
    for d_idx in range(out_d):
        for h_idx in range(out_h):
            for w_idx in range(out_w):
                ...
                # Flatten and store
                patches[:, d_idx, h_idx, w_idx, :] = patch.reshape(
                    batch_size, -1
                )
