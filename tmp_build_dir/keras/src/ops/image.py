...
def extract_volume_patches(
    ...
    """Extracts patches from 3D volumes.
        ...
        Args:
            ...
            size: An integer or tuple of 3 integers `(patch_depth, patch_height, patch_width)`,
                specifying the size of the patches to extract.
    ...
    ```python
        ...
        volume = keras.ops.ones((1, 96, 96, 96, 4))  # (B, D, H, W, C)

        # Extract 4x4x4 patches
        patches = keras.ops.image.extract_volume_patches(
            volume,
            size=(4, 4, 4),
            strides=(4, 4, 4)
        )

        print(patches.shape)  # (1, 24, 24, 24, 256)
    """
...
if len(size) != 3:
    raise ValueError(
        f"size must be an integer or tuple of 3 integers. Received: size={size}"
    )
...
if len(strides) != 3:
    raise ValueError(
        f"strides must be None, an integer or tuple of 3 integers. "
...
if len(dilation_rate) != 3:
    raise ValueError(
        f"dilation_rate must be an integer or tuple of 3 integers. "
