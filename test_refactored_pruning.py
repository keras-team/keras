...
try:
    ...
    print(
        f"   ✓ PruningConfig created: {config.sparsity} sparsity, {config.method} method"
    )
    ...
    print("   ✓ Pruning with config successful!")
    ...
    model2 = keras.Sequential(
        [keras.layers.Dense(32, input_shape=(10,)), keras.layers.Dense(1)]
    )
    model2.compile(optimizer="adam", loss="mse")
    _ = model2(x_dummy)

    stats2 = model2.prune(sparsity=0.3, method="structured")
    print("   ✓ Pruning with kwargs successful!")
