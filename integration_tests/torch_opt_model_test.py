...
def test_opt_model_parallel():
    ...
    for v in backbone.weights:
        if any(name in v.path for name in ["query/kernel", "token_embedding/embeddings", "ffn_inner/kernel", "ffn_outer/kernel"]):
            ...
            is_dtensor = (
                getattr(val, "device_mesh", None) is not None or 
                getattr(val, "placements", None) is not None or
                (hasattr(val, "data") and (
                    getattr(val.data, "device_mesh", None) is not None or 
                    getattr(val.data, "placements", None) is not None
                ))
            )
            if is_dtensor:
                ...
                print(f"  - Placements:   {getattr(local_val, 'placements', 'N/A')}")
            else:
